...
def test_discrete_forward_prices_use_discrete_growth_factors() -> None:
    assert quact.precio_forward(100, 0.05, 2, capitalizacion="Discreta") == pytest.approx(
        100 * 1.05**2
    )
    assert quact.precio_forward_dividendo_continuo(
        100, 0.05, 0.02, 2, capitalizacion="Discreta"
    ) == pytest.approx(100 * 1.05**2 / 1.02**2)
    assert quact.precio_forward_dividendos_discretos(
        100, 0.05, 2, 3, capitalizacion="Discreta"
    ) == pytest.approx(97 * 1.05**2)
    assert quact.precio_forward_commodity(
        100, 0.05, 0.01, 2, capitalizacion="Discreta"
    ) == pytest.approx(100 * 1.05**2 * 1.01**2)
    assert quact.precio_forward_divisa(
        20, 0.08, 0.04, 2, capitalizacion="Discreta"
    ) == pytest.approx(20 * 1.08**2 / 1.04**2)
...
def test_discrete_fra_uses_discrete_forward_rate_and_discounting() -> None:
    forward_rate, value = quact.fra(
        0.045, 0.05, 0.5, 1.0, 1_000_000, 0.055, capitalizacion="Discreta"
    )
