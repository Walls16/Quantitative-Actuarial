...
with tab_inputs:
    ...
    with paso_a_paso():
        st.latex(
            r"n_L = \frac{\alpha_L \cdot n}{1 - R}, \quad n_H = \frac{\alpha_H \cdot n}{1 - R}"
        )
        st.latex(
            rf"n_L = \frac{{{alpha_L:.4f} \times {n_ent}}}{{{LGD:.4f}}} = {n_L:.4f}"
            + r"\quad\Rightarrow\quad m(n_L) = \lceil n_L \rceil = "
            + str(m_nL)
        )
        st.latex(
            rf"n_H = \frac{{{alpha_H:.4f} \times {n_ent}}}{{{LGD:.4f}}} = {n_H:.4f}"
            + r"\quad\Rightarrow\quad m(n_H) = \lceil n_H \rceil = "
            + str(m_nH)
        )
...
with tab_motor:
    ...
    with paso_a_paso():
        st.latex(r"Q(t|F) = N\!\left(\frac{N^{-1}[Q(t)] - \sqrt{\rho}\,F}{\sqrt{1-\rho}}\right)")
        t_ej = _periodos[-1]
    ...
    fig_q.add_vline(
        x=0, line_dash="dash", line_color="gray", line_width=1, annotation_text="F=0 (promedio)"
    )
    ...
    with paso_a_paso():
        st.latex(r"P(k, t|F) = \binom{n}{k}\, Q(t|F)^k \,[1-Q(t|F)]^{n-k}")
        st.latex(r"\log P(k,t|F) = \log\binom{n}{k} + k\log Q + (n-k)\log(1-Q)")
    ...
    st.dataframe(
        df_Ej.style.format({"E_j(F=0)": "{:.8f}"}), use_container_width=True, hide_index=True
    )
    ...
    with paso_a_paso():
        st.latex(r"A(F) = \sum_{j=1}^{m}(\tau_j - \tau_{j-1})\cdot E_j(F)\cdot v(\tau_j)")
        st.latex(
        ...
        st.latex(r"A = \sum_k w_k A(F_k), \quad B = \sum_k w_k B(F_k), \quad C = \sum_k w_k C(F_k)")
...
with tab_resultado:
    ...
    with c1r:
        st.markdown(
            f"<div style='{css_contenedor_col}'><span style='{css_titulo_col}'>A (Fee Leg principal)</span><span style='{css_valor_col}'>{A:.6f}</span></div>",
            unsafe_allow_html=True,
        )
    with c2r:
        st.markdown(
            f"<div style='{css_contenedor_col}'><span style='{css_titulo_col}'>B (Fee Leg accrual)</span><span style='{css_valor_col}'>{B:.6f}</span></div>",
            unsafe_allow_html=True,
        )
    with c3r:
        st.markdown(
            f"<div style='{css_contenedor_col}'><span style='{css_titulo_col}'>C (Protection Leg)</span><span style='{css_valor_col}'>{C:.6f}</span></div>",
            unsafe_allow_html=True,
        )
    with c4r:
        st.markdown(
            f"<div style='{css_contenedor_col}'><span style='{css_titulo_col}'>E(T) — Pérdida esperada</span><span style='{css_valor_col}'>{E_T:.6f}</span></div>",
            unsafe_allow_html=True,
        )
    ...
    with paso_a_paso():
        st.latex(rf"A = {A:.8f}, \quad B = {B:.8f}, \quad C = {C:.8f}")
        st.latex(rf"s = \frac{{C}}{{A+B}} = \frac{{{C:.8f}}}{{{A + B:.8f}}} = {_res['spread']:.8f}")
        themed_success(
    ...
    st.markdown(
        f"#### Pérdida esperada del tranche {_tranche_nombre} por periodo E<sub>j</sub> (promedio sobre F)",
        unsafe_allow_html=True,
    )
...
with tab_todos:
        ...
        filas.append(
            {
                ...
                "Spread (bps)": f"{ri['spread_bps']:.1f}"
                if not np.isnan(ri.get("spread_bps", np.nan))
                else "—",
            }
        )
    ...
    for nom, sp, col in zip(nombres_t, spreads_t, colores_t):
        fig_bar.add_trace(
            go.Bar(
                x=[nom],
                y=[sp if not np.isnan(sp) else 0],
                name=nom,
                marker_color=col,
    ...
    min_tr = min(
        resultados_todos, key=lambda k: resultados_todos[k].get("spread_bps", float("inf"))
    )
...
with tab_export:
    ...
    if st.button("Generar Excel de CDO Tranches", use_container_width=True, key="btn_xls_cdo"):
        try:
            ...
            # Paso 1: Q(t)
            r = section_title(
                ws2, r, "Paso 1 — Probabilidad de default Q(t) = 1 − exp(−h·t)", ncols=4
            )
            ...
            r = section_title(
                ws2, r, "Paso 2 — Prob. condicional Q(t|F=0) = N([N⁻¹(Q) − √ρ·F] / √(1−ρ))", ncols=4
            )
            hdr(ws2, r, 1, "Periodo τ", fill=FIL_AZ2)
            hdr(ws2, r, 2, "Q(τ)", fill=FIL_AZ2)
            ...
            r = section_title(
                ws2,
                r,
                f"Paso 3 — Cuadratura Gauss-Hermite (M={_M} nodos; se muestran los primeros 10)",
                ncols=4,
            )
            ...
            ws2.cell(row=r, column=1, value=f"Suma de todos los {_M} pesos (debe ≈ 1)").font = Font(
                italic=True, size=9
            )
            ...
            r = section_title(
                ws2, r, "Paso 4 — Pérdida esperada E_j(F=0) del tranche por periodo", ncols=4
            )
            hdr(ws2, r, 1, "Periodo τ", fill=FIL_AZ2)
            ...
            r = kv_row(
                ws3,
                r,
                f"SPREAD del tranche {_tranche_nombre}",
                _res["spread_bps"],
                fill_val=FIL_VD2,
                fmt='0.0000 "bps"',
            )
            r = kv_row(
                ws3, r, "Spread (decimal)", _res["spread"], fill_val=FIL_VD2, fmt="0.00000000"
            )
            r += 1
            ...
            r = kv_row(
                ws3, r, "A + B  (denominador del spread)", _res["A"] + _res["B"], fmt="0.00000000"
            )
            ...
            E_T = _res["E_T"]
            r = kv_row(
                ws3, r, "E(T) — fracción del tranche en riesgo al vto.", E_T, fmt="0.00000000"
            )
            r = kv_row(ws3, r, "Pérdida media del tranche (1 − E(T))", 1 - E_T, fmt="0.00%")
            r += 1
            ...
            r = section_title(
                ws3,
                r,
                f"Pérdida esperada E_j por periodo — tranche {_tranche_nombre} (∫ sobre F)",
                ncols=2,
            )
            ...
            for idx, (nom_t, aL_t, aH_t) in enumerate(TRANCHES_DEFAULT):
                ...
                dat(
                    ...
                    r,
                    6,
                    float(ri_t["A"]) if not np.isnan(ri_t["A"]) else None,
                    fill=fill_row,
                    fmt="0.00000000",
                )
                dat(
                    ...
                    r,
                    7,
                    float(ri_t["B"]) if not np.isnan(ri_t["B"]) else None,
                    fill=fill_row,
                    fmt="0.00000000",
                )
                dat(
                    ...
                    r,
                    8,
                    float(ri_t["C"]) if not np.isnan(ri_t["C"]) else None,
                    fill=fill_row,
                    fmt="0.00000000",
                )
                dat(
                    ...
                    r,
                    9,
                    float(ri_t["E_T"]) if not np.isnan(ri_t["E_T"]) else None,
                    fill=fill_row,
                    fmt="0.00000000",
                )
                dat(
                    ...
                    r,
                    10,
                    float(ri_t["spread_bps"]) if not np.isnan(ri_t["spread_bps"]) else None,
            ...
            r += 1
            ws4.cell(
                row=r, column=1, value="* Tranche seleccionado resaltado en verde."
            ).font = Font(italic=True, size=9, color="375623")
            r += 1
            ws4.cell(
                row=r,
                column=1,
                value="Referencia: Hull, J. (2018). Options, Futures, and Other Derivatives, 9ª ed., Cap. 25.",
