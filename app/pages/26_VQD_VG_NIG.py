...
st.markdown(
    '<div class="page-sub">Modelos de actividad infinita basados en procesos de Levy. Capturan colas pesadas, asimetria y kurtosis sin saltos discretos de Poisson.</div>',
    unsafe_allow_html=True,
)
...
st.markdown(
    f"<div style='font-size:0.72rem;letter-spacing:0.14em;text-transform:uppercase;color:{text_sub};margin-bottom:0.6rem'>Parametros comunes</div>",
    unsafe_allow_html=True,
)
...
with col_vg:
    st.markdown(
        f"<div style='font-size:0.72rem;letter-spacing:0.14em;text-transform:uppercase;color:{accent};margin:0.8rem 0 0.4rem'>Parametros VG</div>",
        unsafe_allow_html=True,
    )
    ...
    vg_sigma = st.slider(
        "sigma VG (vol difusiva)", 0.01, 1.0, float(_pvg.get("sigma", 0.12)), 0.01, key="vgs"
    )
    vg_theta = st.slider(
        "theta VG (sesgo)", -1.0, 1.0, float(_pvg.get("theta", -0.14)), 0.01, key="vgt"
    )
    vg_nu = st.slider("nu VG (kurtosis)", 0.01, 2.0, float(_pvg.get("nu", 0.20)), 0.01, key="vgn")

with col_nig:
    st.markdown(
        f"<div style='font-size:0.72rem;letter-spacing:0.14em;text-transform:uppercase;color:{accent2};margin:0.8rem 0 0.4rem'>Parametros NIG</div>",
        unsafe_allow_html=True,
    )
    ...
    nig_alpha = st.slider(
        "alpha NIG (colas)", 1.0, 50.0, float(_pnig.get("alpha", 15.0)), 0.5, key="nia"
    )
    nig_beta = st.slider(
        "beta NIG (sesgo)",
        float(-nig_alpha + 0.1),
        float(nig_alpha - 0.1),
        float(np.clip(_pnig.get("beta", -5.0), -nig_alpha + 0.01, nig_alpha - 0.01)),
        0.1,
        key="nib",
    )
    nig_delta = st.slider(
        "delta NIG (escala)", 0.1, 5.0, float(_pnig.get("delta", 1.5)), 0.1, key="nid"
    )
...
with st.spinner("Calculando VG y NIG via FFT..."):
    ...
    vg_call = vg.call_price()
    vg_put = vg.put_price()
    nig_call = nig.call_price()
    nig_put = nig.put_price()
    bsm_call = bsm.call_price()
    bsm_put = bsm.put_price()

st.divider()
st.markdown(
    f"<div style='font-size:0.72rem;letter-spacing:0.14em;text-transform:uppercase;color:{text_sub};margin-bottom:0.6rem'>Resultados</div>",
    unsafe_allow_html=True,
)

rc = st.columns(5)
for col_r, (label, c_val, p_val, ref_c, color) in zip(
    rc,
    [
...
tab1, tab2, tab3 = st.tabs(
    ["Sonrisas VG vs NIG", "Distribucion de retornos", "Sensibilidad parametrica"]
)
...
with tab1:
    ...
    fig_sm = go.Figure()
    fig_sm.add_trace(
        go.Scatter(x=strikes_sm, y=ivs_vg, name="VG IV", line=dict(color=accent, width=2.5))
    )
    fig_sm.add_trace(
        go.Scatter(x=strikes_sm, y=ivs_nig, name="NIG IV", line=dict(color=accent2, width=2.5))
    )
    fig_sm.add_trace(
        go.Scatter(
            x=strikes_sm,
    ...
    fig_sm.update_layout(
        **plotly_layout("Sonrisas de Volatilidad Implicita — VG vs NIG vs BSM"), height=400
    )
...
with tab2:
    st.markdown(
        f"<div style='font-size:0.82rem;color:{text_sub};margin-bottom:0.8rem'>Distribucion simulada de log-retornos bajo cada modelo vs Normal (GBM).</div>",
        unsafe_allow_html=True,
    )

    dist = simulate_vg_nig_returns(
        T,
        vg_sigma,
        vg_theta,
        vg_nu,
    ...
    vg_returns = dist["vg_returns"]
    nig_returns = dist["nig_returns"]
    gbm_returns = dist["gbm_returns"]
    m_vg = dist["moments"]["VG"]
    m_nig = dist["moments"]["NIG"]
    m_gbm = dist["moments"]["GBM"]

    col_mom = st.columns(3)
    for col_m, (name, m, color) in zip(
        col_mom,
        [
    ...
    st.caption(
        "Colas mas pesadas y mayor asimetria que la Normal = mayor precio para opciones OTM."
    )
...
with tab3:
    ...
    with col_s1:
        st.markdown(
            f"<div style='font-size:0.72rem;color:{text_sub};text-transform:uppercase;margin-bottom:0.4rem'>VG: precio call vs nu (kurtosis)</div>",
            unsafe_allow_html=True,
        )
        ...
        fig_vgnu.add_vline(
            x=vg_nu, line_dash="dot", line_color=text_sub, annotation_text="nu actual"
        )
    ...
    with col_s2:
        st.markdown(
            f"<div style='font-size:0.72rem;color:{text_sub};text-transform:uppercase;margin-bottom:0.4rem'>NIG: precio call vs beta (sesgo)</div>",
            unsafe_allow_html=True,
        )
        ...
        fig_nigb = go.Figure()
        fig_nigb.add_trace(
            go.Scatter(x=betas_range, y=nig_calls_b, line=dict(color=accent2, width=2))
        )
        fig_nigb.add_hline(y=bsm_call, line_dash="dot", line_color="#f87171", annotation_text="BSM")
        fig_nigb.add_vline(
            x=nig_beta, line_dash="dot", line_color=text_sub, annotation_text="beta actual"
        )
