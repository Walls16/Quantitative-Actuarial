...
st.markdown(
    '<div class="page-sub">El modelo mas antiguo de valuacion de opciones (1900) y su relacion con el modelo normal moderno. Mas Monte Carlo con reduccion de varianza e intervalos de confianza.</div>',
    unsafe_allow_html=True,
)
...
# ── Section 1: Bachelier ──
st.markdown(
    f"<div style='font-size:0.78rem;font-family:JetBrains Mono,monospace;letter-spacing:0.12em;text-transform:uppercase;color:{accent};margin-bottom:0.8rem'>Modelo de Bachelier</div>",
    unsafe_allow_html=True,
)
...
with col_in:
    ...
    sigma_n = st.number_input(
        "sigma_n — vol normal (unidades absolutas, ej. $20 = 20% de S)",
        value=20.0,
        min_value=0.01,
        max_value=S * 5,
        step=0.5,
    ...
    sigma_ln = (
        st.number_input(
            "sigma_LN — vol lognormal BSM (%) para comparar",
            value=20.0,
            min_value=0.01,
            max_value=200.0,
            step=0.5,
...
bch_call = bch.call_price()
bch_put = bch.put_price()
...
with col_out:
    cols_r = st.columns(4)
    for col_r, (label, val, ref, color) in zip(
        cols_r,
        [
...
st.markdown(
    f"<div style='font-size:0.78rem;font-family:JetBrains Mono,monospace;letter-spacing:0.12em;text-transform:uppercase;color:{accent2};margin-bottom:0.8rem'>Monte Carlo con reduccion de varianza e intervalos de confianza</div>",
    unsafe_allow_html=True,
)
...
with col_mc1:
    st.markdown(
        f"<div style='font-size:0.72rem;color:{text_sub};text-transform:uppercase;letter-spacing:0.1em;margin-bottom:0.5rem'>Configuracion MC</div>",
        unsafe_allow_html=True,
    )
    mc_type = st.radio("Tipo de opcion", ["call", "put"], horizontal=True)
    n_paths = st.select_slider(
        "Numero de trayectorias", [1_000, 5_000, 10_000, 50_000, 100_000, 500_000], value=50_000
    )
...
tab1, tab2, tab3 = st.tabs(
    ["Bachelier vs BSM — perfiles", "Convergencia MC", "Comparativa tecnicas MC"]
)
...
with tab1:
    ...
    with col_pb2:
        ...
        fig_p.add_trace(
            go.Scatter(
                x=strikes_plot, y=bch_puts, name="Bachelier Put", line=dict(color=accent, width=2.5)
            )
        )
        fig_p.add_trace(
            go.Scatter(
    ...
    # Delta comparison
    bch_deltas = [
        BachelierEngine(S, k, T, r, sigma_n, q).greeks()["call_delta"] for k in strikes_plot
    ]
    ...
    fig_d.add_trace(
        go.Scatter(
            x=strikes_plot, y=bch_deltas, name="Bachelier Delta", line=dict(color=accent, width=2)
        )
    )
    fig_d.add_trace(
        go.Scatter(
...
with tab2:
    ...
    # Error convergence
    fig_err = go.Figure()
    theo_err = theoretical_mc_error(bsm_analytical, mc_sigma, T, ns_conv)
    fig_err.add_trace(
        go.Scatter(
            x=ns_conv,
            y=mc_errs_conv,
    ...
    st.caption(
        "El error converge como O(1/sqrt(N)). Duplicar la precision requiere 4x mas trayectorias."
    )

with tab3:
    st.markdown(
        f"<div style='font-size:0.82rem;color:{text_sub};margin-bottom:0.8rem'>Comparacion de precision entre las 4 combinaciones de tecnicas de reduccion de varianza con N fijo.</div>",
        unsafe_allow_html=True,
    )
    ...
    for col_t, (name_mc, res) in zip(cols_mc_tech, results_mc):
        eff = (
            results_mc[0][1]["std_error"] / res["std_error"]
            if res["std_error"] > 0
            else float("inf")
        )
        col_t.markdown(
    ...
    st.caption(
        f"Eficiencia = SE_ingenuo / SE_tecnica. La variable de control (BSM) es la mas efectiva para opciones europeas bajo GBM — la solucion analitica elimina casi todo el error de estimacion."
    )
