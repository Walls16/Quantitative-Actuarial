...
st.markdown(
    '<div class="page-eyebrow">Datos en Tiempo Real · Página 7</div>', unsafe_allow_html=True
)
st.markdown('<div class="page-title">Subyacentes en Vivo</div>', unsafe_allow_html=True)
st.markdown(
    '<div class="page-sub">Valuación de opciones sobre datos reales de mercado vía yfinance. Extrae precio spot, cadena de opciones e IV implícita del mercado.</div>',
    unsafe_allow_html=True,
)
...
with col_r_input:
    r_manual = (
        st.number_input("Tasa libre de riesgo (r, %)", value=5.25, step=0.25, format="%.2f") / 100
    )
...
if not hist_long.empty and not _close_long.dropna().empty:
    fig_hist = go.Figure()
    colors_hist = [
        positive if c >= o else negative for c, o in zip(_close_long.ffill(), _open_long.ffill())
    ]
    fig_hist.add_trace(
        go.Scatter(
            x=_date_long,
    ...
    fig_hist.update_layout(
        **plotly_layout(f"{ticker_input} — Precio histórico (6 meses)"), height=260
    )
...
if chain_err or calls_df is None or calls_df.empty:
    st.error(
        f"Error cargando cadena de opciones para {exp_choice}. Prueba otra fecha de vencimiento."
    )
    st.stop()
...
if df_chain.empty:
    st.error(
        f"La cadena de opciones de {ticker_input} / {exp_choice} no tiene strikes disponibles. Prueba otra fecha."
    )
    st.stop()
...
atm_iv_auto = (
    _iv_auto_raw if (not isinstance(_iv_auto_raw, float) or not np.isnan(_iv_auto_raw)) else None
)

#  Strike selector
st.markdown(
    f"<div class='chain-header'>Valuación de opción — {ticker_input}</div>", unsafe_allow_html=True
)
...
if opt_type_live == "call":
    _mono = (
        "ATM"
        if abs(chosen_strike - spot) / spot < 0.015
        else ("ITM" if spot > chosen_strike else "OTM")
    )
else:
    _mono = (
        "ATM"
...
#  Info bar
info_parts = [
    f"T = {T_live * 365:.0f} días ({T_live:.4f} años)",
    f"r = {r_manual * 100:.2f}%",
    f"σ = {atm_sigma * 100:.2f}%",
]
if market_mid is not None:
    info_parts.insert(0, f"Precio mkt (mid, K≈{ref_strike_label:.2f}) = ${market_mid:.3f}")
if ref_iv:
    info_parts.insert(
        1 if market_mid is None else 1,
        f"IV cadena (K={ref_strike_label:.2f}) = {ref_iv * 100:.2f}%",
    )
st.markdown(
...
tab1, tab2, tab3, tab4 = st.tabs(
    ["Cadena de opciones", "Sonrisa IV de mercado", "Valuación por strike", "Vol histórica vs IV"]
)
...
with tab1:
    ...
    df_display.columns = [
        c.replace("IV_market", "IV_calc (%)")
        .replace("impliedvolatility", "IV_yahoo")
        .replace("midPrice", "Mid")
    ...
    if len(df_iv) < 3:
        ...
        if "impliedvolatility" in df_chain.columns:
            ...
            fig_iv.add_trace(
                go.Scatter(
                    x=df_yahoo["strike"],
                    y=df_yahoo["impliedvolatility"] * 100,
                    mode="markers",
        ...
        fig_iv.add_vline(
            x=spot, line_dash="dash", line_color=positive, annotation_text=f"S={spot:.2f}"
        )
        fig_iv.add_vline(x=atm_strike, line_dash="dot", line_color="#f59e0b", annotation_text="ATM")
        fig_iv.update_layout(
            **plotly_layout(f"Sonrisa de Volatilidad Implícita — {ticker_input} · {exp_choice}"),
            height=400,
        )
        ...
        if "openinterest" in df_chain.columns:
            ...
            if not df_oi.empty:
                ...
                fig_oi.add_trace(
                    go.Bar(
                        x=df_oi["strike"],
                        y=df_oi["openinterest"],
                        name="Open Interest",
                        marker_color=[
                            accent if abs(k - spot) / spot < 0.05 else grid_col
                            for k in df_oi["strike"]
                        ],
                    )
                )
                fig_oi.add_vline(
                    x=spot, line_dash="dash", line_color=positive, annotation_text=f"S"
                )
                fig_oi.update_layout(
                    **plotly_layout(f"Open Interest por Strike — {opt_type_live.upper()}"),
                    height=280,
                )
...
with tab3:
    st.markdown(
        f"<div style='font-size:0.82rem;color:{text_sub};margin-bottom:0.8rem'>Valuación de todos los modelos a través de la cadena de strikes disponibles (usando IV ATM como σ base).</div>",
        unsafe_allow_html=True,
    )
    ...
    with st.spinner("Calculando valuación por strike..."):
        bsm_live = [
            BSMEngine(spot, k, T_live, r_manual, atm_sigma, q_live).call_price()
            if opt_type_live == "call"
            else BSMEngine(spot, k, T_live, r_manual, atm_sigma, q_live).put_price()
            for k in strikes_live
        ]
        merton_live = [
            MertonEngine(
                spot, k, T_live, r_manual, atm_sigma, q_live, 1.0, -0.05, 0.15
            ).call_price()
            if opt_type_live == "call"
            else MertonEngine(
                spot, k, T_live, r_manual, atm_sigma, q_live, 1.0, -0.05, 0.15
    ...
    fig_chain.add_trace(
        go.Scatter(
            ...
            y=bsm_live,
            name="BSM",
            line=dict(color=MODEL_COLORS["BSM"], width=2, dash="dash"),
        )
    )
    fig_chain.add_trace(
        go.Scatter(
            ...
            y=merton_live,
            name="Merton",
            line=dict(color=MODEL_COLORS["Merton"], width=2),
        )
    )
    ...
    fig_chain.update_layout(
        **plotly_layout(
            f"Modelos vs Mercado — {ticker_input} {opt_type_live.upper()} · {exp_choice}"
        ),
        height=420,
    )
    ...
    st.caption(
        f"σ base usada = {atm_sigma * 100:.2f}% (IV ATM) · T = {T_live * 365:.0f} días · r = {r_manual * 100:.2f}%"
    )
...
with tab4:
    ...
    with col_rv1:
        ...
        rv_period = st.selectbox(
            "Periodo historico", ["3mo", "6mo", "1y", "2y"], index=1, key="rv_per"
        )
    ...
    if close_rv is None or len(close_rv) < rv_window + 5:
        ...
        with col_rv2:
            ...
            _rvl = dict(
                ...
                font=dict(family="Inter", color=text_main, size=12),
                title=dict(
                    text=f"{ticker_input} — Volatilidad Realizada vs IV de Mercado",
        ...
        if len(rv_valid_arr) > 0:
            ...
            st.markdown(
                f"<div style='font-size:0.72rem;color:{text_sub};text-transform:uppercase;letter-spacing:0.1em;margin:0.8rem 0 0.4rem'>Estadisticos del spread IV-RV</div>",
                unsafe_allow_html=True,
            )
