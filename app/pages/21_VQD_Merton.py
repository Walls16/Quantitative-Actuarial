...
st.markdown(
    '<div class="page-sub">Proceso difusivo con saltos de Poisson log-normales. Captura eventos de cola, skew de volatilidad y discontinuidades de precio.</div>',
    unsafe_allow_html=True,
)
...
with col_in:
    st.markdown(
        f"<div style='font-size:0.72rem;letter-spacing:0.14em;text-transform:uppercase;color:{text_sub};margin-bottom:0.8rem'>Parámetros del subyacente</div>",
        unsafe_allow_html=True,
    )
    S = st.number_input(
        "Precio subyacente (S)", value=100.0, min_value=0.01, step=1.0, format="%.2f"
    )
    K = st.number_input(
        "Precio de ejercicio (K)", value=100.0, min_value=0.01, step=1.0, format="%.2f"
    )
    T = st.number_input(
    ...
    st.markdown(
        f"<div style='font-size:0.72rem;letter-spacing:0.14em;text-transform:uppercase;color:{text_sub};margin:1rem 0 0.5rem 0'>Parámetros de salto</div>",
        unsafe_allow_html=True,
    )
    _pm = PRESETS_M[preset_m] or {}
    lam = st.slider(
        "λ — intensidad de saltos (por año)", 0.0, 10.0, float(_pm.get("lam", 1.0)), 0.1
    )
    mu_j = st.slider(
        "μ — tamaño medio del salto (log)", -1.0, 1.0, float(_pm.get("mu_j", -0.1)), 0.01
    )
    sigma_j = st.slider(
        "σ — volatilidad del salto", 0.01, 1.0, float(_pm.get("sigma_j", 0.15)), 0.01
    )
    n_terms = st.slider("Términos de la serie", 10, 100, 50, 5)

    jump_stats = merton_jump_statistics(lam, mu_j, sigma_j, sigma)
    kappa_j = jump_stats["kappa_j"]
    mean_jump_pct = jump_stats["mean_jump_pct"]
...
bsm_total_sigma = jump_stats["bsm_total_sigma"]
...
with col_out:
    st.markdown(
        f"<div style='font-size:0.72rem;letter-spacing:0.14em;text-transform:uppercase;color:{text_sub};margin-bottom:0.8rem'>Resultados</div>",
        unsafe_allow_html=True,
    )
...
with tab2:
    ...
    with col_l:
        ...
        calls_lam = [
            MertonEngine(S, K, T, r, sigma, q, l, mu_j, sigma_j).call_price() for l in lam_range
        ]
        fig_l = go.Figure()
    ...
    with col_s:
        ...
        calls_sj = [
            MertonEngine(S, K, T, r, sigma, q, lam, mu_j, sj).call_price() for sj in sj_range
        ]
        fig_s = go.Figure()
        fig_s.add_trace(
            go.Scatter(x=sj_range * 100, y=calls_sj, line=dict(color="#a78bfa", width=2))
        )
        fig_s.add_vline(
            x=sigma_j * 100, line_dash="dot", line_color=text_sub, annotation_text="σ actual"
        )
...
with tab3:
    st.markdown(
        f"<div style='font-size:0.8rem;color:{text_sub};margin-bottom:0.8rem'>Simulación de Monte Carlo de trayectorias bajo el proceso Merton jump-diffusion</div>",
        unsafe_allow_html=True,
    )
    ...
    fig_mc = go.Figure()
    merton_sim = simulate_merton_paths(
        S, T, r, sigma, q, lam, mu_j, sigma_j, n_paths=n_paths, n_steps=252, seed=42
    )
    times = merton_sim["times"]
    merton_paths = merton_sim["paths"]
    terminal_prices = merton_sim["terminal_prices"]
    for i, S_path in enumerate(merton_paths):
        color = (
            f"rgba(79,142,247,{0.2 + 0.5 * (i / n_paths)})"
            if dark
            else f"rgba(37,99,235,{0.2 + 0.5 * (i / n_paths)})"
        )
        fig_mc.add_trace(
            go.Scatter(
                x=times, y=S_path, mode="lines", line=dict(width=1, color=color), showlegend=False
            )
        )

    if compare_gbm:
        gbm_sim = simulate_gbm_paths(
            S, T, r, sigma, q, n_paths=min(n_paths, 15), n_steps=252, seed=99
        )
        for i, S_gbm in enumerate(gbm_sim["paths"]):
            fig_mc.add_trace(
                go.Scatter(
                    x=times,
                    y=S_gbm,
    ...
    fig_mc.update_layout(
        **plotly_layout(f"Trayectorias — Merton (λ={lam}, μ={mu_j}, σ={sigma_j})"), height=400
    )
    ...
    if show_histogram:
        ...
        if compare_gbm:
            gbm_terminal = simulate_gbm_paths(S, T, r, sigma, q, n_paths=500, n_steps=1, seed=77)[
                "terminal_prices"
            ]
            fig_hist.add_trace(
                go.Histogram(
        ...
        fig_hist.update_layout(
            **plotly_layout("Distribución de S(T) — Merton vs GBM"), barmode="overlay", height=300
        )
        fig_hist.update_xaxes(title="S(T)")
        fig_hist.update_yaxes(title="Frecuencia")
        kurt_mc = excess_kurtosis(terminal_prices)
        st.plotly_chart(fig_hist, use_container_width=True)
        st.caption(
            f"Kurtosis exceso Merton ≈ {kurt_mc:.2f} (Normal = 0). Cola pesada → kurtosis positiva."
        )
