...
st.markdown(
    '<div class="page-sub">Hagan, Kumar, Lesniewski & Woodward (2002). Estandar de mercado en renta fija y FX. Produce smiles y skews sin necesidad de calibrar una vol por strike.</div>',
    unsafe_allow_html=True,
)
...
with col_in:
    st.markdown(
        f"<div style='font-size:0.72rem;letter-spacing:0.14em;text-transform:uppercase;color:{text_sub};margin-bottom:0.8rem'>Parametros</div>",
        unsafe_allow_html=True,
    )
    ...
    F = forward_price_continuous(S, r, q, T)

    st.markdown(
        f"<div style='font-size:0.72rem;letter-spacing:0.14em;text-transform:uppercase;color:{text_sub};margin:0.8rem 0 0.4rem'>Parametros SABR</div>",
        unsafe_allow_html=True,
    )
    ...
    F_val = forward_price_continuous(S, r, q, T)
    st.markdown(
        f"""
...
with col_out:
    ...
    m3.metric(
        "Diff Call vs BSM(alpha)", f"${call - BSMEngine(S, K, T, r, alpha, q).call_price():.4f}"
    )

st.divider()

tab1, tab2, tab3, tab4 = st.tabs(
    ["Sonrisa SABR", "Efecto de parametros", "Calibracion manual", "Comparativa beta"]
)
...
with tab1:
    ...
    fig = go.Figure()
    fig.add_trace(
        go.Scatter(x=x_ax, y=ivs_sabr, name="SABR IV", line=dict(color=accent, width=2.5))
    )
    fig.add_trace(
        go.Scatter(
            x=x_ax,
            y=ivs_flat,
            name="alpha (nivel plano)",
...
with tab2:
    ...
    with col_p1:
        ...
        for r_val in rhos:
            ...
            fig_r.add_trace(
                go.Scatter(x=strikes_plot, y=ivs, name=f"rho={r_val:.1f}", line=dict(width=1.5))
            )
    ...
    with col_p2:
        ...
        for n_val in nus:
            ...
            ivs = s_t.vol_smile(strikes_plot) * 100
            fig_n.add_trace(
                go.Scatter(x=strikes_plot, y=ivs, name=f"nu={n_val}", line=dict(width=1.5))
            )
...
with tab3:
    st.markdown(
        f"<div style='font-size:0.85rem;color:{text_sub};margin-bottom:1rem'>Ingresa puntos de la sonrisa de mercado observada (strike, IV%) para calibrar alpha, rho y nu con beta fijo.</div>",
        unsafe_allow_html=True,
    )

    st.markdown(
        f"<div style='font-size:0.72rem;color:{text_sub};text-transform:uppercase;letter-spacing:0.1em;margin-bottom:0.5rem'>Puntos de mercado (strike, IV%)</div>",
        unsafe_allow_html=True,
    )
    ...
    default_ivs_mkt = [22.0, 20.5, 20.0, 20.5, 21.5][:n_points] + [20.0] * (
        n_points - min(n_points, 5)
    )
    ...
    for i in range(n_points):
        ...
        with col:
            ...
            iv_inp = st.number_input(
                f"IV_{i + 1} (%)",
                value=float(default_ivs_mkt[i] if i < len(default_ivs_mkt) else 20.0),
                min_value=0.1,
                max_value=200.0,
                step=0.1,
                format="%.2f",
    ...
    if st.button("Calibrar SABR"):
        with st.spinner("Calibrando..."):
            result = calibrate_sabr(F_val, T, mkt_strikes, mkt_ivs, beta=beta_cal)
        ...
        sabr_cal = SABREngine(
            F_val, K, T, result["alpha"], beta_cal, result["rho"], result["nu"], r, q, S
        )
...
with tab4:
    ...
    for b_val, b_col in zip(betas_compare, beta_colors):
        ...
        label = (
            f"beta={b_val} ({'Normal' if b_val == 0 else ('Lognormal' if b_val == 1 else 'CEV')})"
        )
        fig_b.add_trace(
            go.Scatter(x=strikes_plot, y=ivs_b, name=label, line=dict(color=b_col, width=2))
        )
    ...
    st.caption(
        "beta=0: proceso normal (Bachelier). beta=1: proceso lognormal (BSM). Beta afecta la pendiente estructural del skew independientemente de rho."
    )
