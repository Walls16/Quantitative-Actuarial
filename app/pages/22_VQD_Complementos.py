...
st.markdown(
    '<div class="page-sub">Griegas de primer y segundo orden, sonrisa de volatilidad, superficie IV, IV implícita desde precio de mercado, paridad put-call.</div>',
    unsafe_allow_html=True,
)
...
with st.expander(" Parámetros base", expanded=True):
    ...
    with c2:
        T = st.number_input(
            "T — tiempo (años)", value=1.0, min_value=0.01, step=0.05, format="%.3f"
        )
        r = st.number_input("r — tasa (%)", value=5.0, step=0.25, format="%.2f") / 100
    with c3:
        sigma = (
            st.number_input(
                "σ — volatilidad (%)", value=20.0, min_value=0.1, step=0.5, format="%.2f"
            )
            / 100
        )
...
tab1, tab2, tab3, tab4 = st.tabs(
    ["Griegas completas", "Sonrisa de volatilidad", "IV implícita", "Paridad put-call"]
)
...
with tab1:
    ...
    st.markdown(
        '<div class="section-header">Griegas de segundo orden</div>', unsafe_allow_html=True
    )
    cols2 = st.columns(3)
    greeks_2nd = [
    ...
    greek_choice = st.selectbox(
        "Graficar griega", ["Delta", "Gamma", "Theta", "Vega", "Rho", "Vanna", "Vomma"]
    )
    ...
    fig_g.add_trace(
        go.Scatter(
            x=xs, y=vals_call, name=f"{greek_choice} (Call)", line=dict(color=accent, width=2)
        )
    )
    if fn_put:
        ...
        fig_g.add_trace(
            go.Scatter(
                x=xs, y=vals_put, name=f"{greek_choice} (Put)", line=dict(color="#a78bfa", width=2)
            )
        )
    ...
    st.markdown(
        f"<div style='font-size:0.72rem;color:{text_sub};text-transform:uppercase;letter-spacing:0.1em;margin:1.2rem 0 0.5rem 0'>Heatmap — {greek_choice} sobre grilla S × σ</div>",
        unsafe_allow_html=True,
    )
    ...
    fig_hm.add_hline(
        y=sigma * 100, line_dash="dot", line_color=accent, annotation_text=f"σ={sigma * 100:.0f}%"
    )
...
with tab2:
    st.markdown(
        f"<div style='font-size:0.82rem;color:{text_sub};margin-bottom:0.8rem'>Volatilidad implícita derivada de precios BSM. En un mundo BSM puro la superficie es plana; en mercados reales, inclinada (skew) o en forma de sonrisa.</div>",
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        '<div class="section-header">Superficie IV (maturity × strike)</div>',
        unsafe_allow_html=True,
    )
...
with tab3:
    st.markdown(
        f"<div style='font-size:0.82rem;color:{text_sub};margin-bottom:1rem'>Dado un precio de mercado observado, calcula la volatilidad implícita que lo reproduce bajo BSM. Método: Brent (bisección acelerada).</div>",
        unsafe_allow_html=True,
    )
    ...
    if not np.isnan(iv_result):
        ...
        check_price = (
            BSMEngine(S, K, T, r, iv_result, q).call_price()
            if opt_type_iv == "call"
            else BSMEngine(S, K, T, r, iv_result, q).put_price()
        )
        st.markdown(
            f"""
...
with tab4:
    st.markdown(
        f"<div style='font-size:0.82rem;color:{text_sub};margin-bottom:1rem'>La paridad put-call es una relación de no-arbitraje fundamental: C − P = S·e<sup>−qT</sup> − K·e<sup>−rT</sup></div>",
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        '<div class="section-header">Paridad a través de strikes</div>', unsafe_allow_html=True
    )
    ...
    fig_par = go.Figure()
    fig_par.add_trace(
        go.Scatter(x=strikes_par, y=lhs_arr, name="C − P", line=dict(color=accent, width=2))
    )
    fig_par.add_trace(
        go.Scatter(
            x=strikes_par,
