...
st.markdown(
    '<div class="page-sub">Valuación discreta mediante árbol binomial recombinante. Soporta opciones americanas con ejercicio anticipado.</div>',
    unsafe_allow_html=True,
)
...
with col_in:
    ...
    tree_params = crr_tree_parameters(T, r, sigma, q, N)
    dt = tree_params["dt"]
    u = tree_params["u"]
    d = tree_params["d"]
    p = tree_params["p"]
...
with tab1:
    ...
    with col_t1c:
        tree_show = st.radio(
            "Mostrar en nodos", ["Valor opción", "Precio S", "Ambos"], horizontal=False
        )
    ...
    fig_tree.add_trace(
        go.Scatter(
            ...
            marker=dict(
                size=32,
                color=node_color,
                colorscale="Blues" if not dark else "Blues",
                showscale=True,
...
with tab2:
    ...
    fig2 = go.Figure()
    fig2.add_trace(
        go.Scatter(x=Ns, y=conv_calls, name="CRR Call", line=dict(color=accent, width=2))
    )
    fig2.add_trace(
        go.Scatter(x=Ns, y=conv_puts, name="CRR Put", line=dict(color="#a78bfa", width=2))
    )
    fig2.add_hline(
        y=bsm_call,
        line_dash="dot",
        line_color="#34d399",
        annotation_text="BSM Call",
    ...
    if show_error:
        ...
        fig2e.add_trace(
            go.Scatter(
                x=Ns, y=err_calls, name="|CRR Call - BSM|", line=dict(color=accent, width=1.5)
            )
        )
        fig2e.add_trace(
            go.Scatter(
                x=Ns, y=err_puts, name="|CRR Put - BSM|", line=dict(color="#a78bfa", width=1.5)
            )
        )
        ...
        st.caption(
            f"Oscilación par/impar visible en N pequeño. A N={N}: error call = ${abs(call_crr - bsm_call):.4f}"
        )
...
with tab3:
    ...
    fig3 = go.Figure()
    fig3.add_trace(
        go.Scatter(x=strikes, y=crr_c, name="CRR Call", line=dict(color=accent, width=2))
    )
    fig3.add_trace(
        go.Scatter(
            x=strikes, y=bsm_c, name="BSM Call", line=dict(color="#34d399", width=2, dash="dash")
        )
    )
    fig3.add_trace(
        go.Bar(
