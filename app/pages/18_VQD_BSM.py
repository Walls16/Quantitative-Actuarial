...
#  Theme: VQD is fixed to light mode.
dark = False
...
st.markdown(
    '<div class="page-sub">Fórmula cerrada para la valuación de opciones europeas bajo volatilidad constante.</div>',
    unsafe_allow_html=True,
)
...
with col_in:
    st.markdown(
        f"<div style='font-size:0.72rem;letter-spacing:0.14em;text-transform:uppercase;color:{text_sub};margin-bottom:0.8rem'>Parámetros</div>",
        unsafe_allow_html=True,
    )

    S = st.number_input(
        "Precio del subyacente (S)", value=100.0, min_value=0.01, step=1.0, format="%.2f"
    )
    K = st.number_input(
    ...
    d_summary = bsm_d1_d2_summary(S, K, T, r, sigma, q)
    st.markdown(
        f"""
...
with col_out:
    ...
    st.markdown(
        f"<span style='background:{color_m}22;color:{color_m};border:1px solid {color_m}44;border-radius:4px;padding:0.2rem 0.6rem;font-size:0.78rem;font-family:JetBrains Mono,monospace'>{moneyness} · S/K = {S / K:.3f}</span>",
        unsafe_allow_html=True,
    )
...
with tab1:
    ...
    strikes = np.linspace(
        max(S * (strike_range_pct / 100), 1), S * (2 - strike_range_pct / 100), 300
    )
    ...
    fig = go.Figure()
    fig.add_trace(
        go.Scatter(x=strikes, y=calls_v, name="Call (valor)", line=dict(color=accent, width=2.5))
    )
    fig.add_trace(
        go.Scatter(x=strikes, y=puts_v, name="Put (valor)", line=dict(color="#a78bfa", width=2.5))
    )
    if show_payoff:
        fig.add_trace(
            go.Scatter(
    ...
    # Scenario table
    st.markdown(
        f"<div style='font-size:0.72rem;letter-spacing:0.12em;text-transform:uppercase;color:{text_sub};margin:0.5rem 0 0.4rem 0'>Escenarios de precio al vencimiento</div>",
        unsafe_allow_html=True,
    )
    scenarios = {
        "S×0.70": S * 0.70,
...
with tab2:
    ...
    with col_t2a:
        param_choice = st.selectbox(
            "Variar parámetro",
            ["Precio subyacente (S)", "Volatilidad (σ)", "Tiempo (T)", "Tasa (r)"],
        )
    with col_t2b:
    ...
    if overlay_scenarios and param_choice == "Volatilidad (σ)":
        ...
        fig2b.add_trace(
            go.Scatter(x=xs * 100, y=calls_p, name="Call", line=dict(color=accent, width=2))
        )
        fig2b.add_trace(
            go.Scatter(x=xs * 100, y=puts_p, name="Put", line=dict(color="#a78bfa", width=2))
        )
        fig2b.update_layout(
            **plotly_layout("Sensibilidad call/put con σ actual resaltada"), height=300
        )
        ...
        for t_ov, col_ov in zip(
            [0.08, 0.25, 0.5, 1.0, 2.0], [text_sub, "#a78bfa", "#f59e0b", accent, "#34d399"]
        ):
...
with tab3:
    ...
    with col_g1:
        st.markdown(
            f"<div style='font-size:0.72rem;color:{text_sub};text-transform:uppercase;letter-spacing:0.1em;margin-bottom:0.6rem'>Griegas de 1.er orden</div>",
            unsafe_allow_html=True,
        )
        m1, m2 = st.columns(2)
    ...
    with col_g2:
        st.markdown(
            f"<div style='font-size:0.72rem;color:{text_sub};text-transform:uppercase;letter-spacing:0.1em;margin-bottom:0.6rem'>Griegas de 2.° orden</div>",
            unsafe_allow_html=True,
        )
        m1, m2 = st.columns(2)
    ...
    st.markdown(
        f"<div style='font-size:0.72rem;color:{text_sub};text-transform:uppercase;letter-spacing:0.1em;margin:1rem 0 0.4rem 0'>Delta vs precio subyacente</div>",
        unsafe_allow_html=True,
    )
    ...
    fig3 = go.Figure()
    fig3.add_trace(
        go.Scatter(x=xs_d, y=call_deltas, name="Δ Call", line=dict(color=accent, width=2))
    )
    fig3.add_trace(
        go.Scatter(x=xs_d, y=put_deltas, name="Δ Put", line=dict(color="#a78bfa", width=2))
    )
    fig3.add_trace(
        go.Scatter(
            x=xs_d,
