...
with tab_gordon:
    ...
    with c1:
        ...
        k_gs = (
            st.number_input(
                "Tasa de rendimiento requerida ($k$) %", value=12.0, step=0.1, key="gs_k"
            )
            / 100
        )
        g_gs = (
            st.number_input(
                "Tasa de crecimiento constante ($g$) %", value=4.0, step=0.1, key="gs_g"
            )
            / 100
        )
    ...
    with c2:
        if k_gs <= g_gs:
            ...
            with paso_a_paso():
                ...
                themed_success(
                    f"<div style='{css_paso}'><span style='{math_style}'>P<sub>0</sub></span> = ${precio_gs:,.4f}</div>"
                )
...
with tab_rendimiento:
    ...
    with c1:
        p0_rr = st.number_input(
            "Precio actual de la acción ($P_0$)", min_value=0.01, value=150.0, step=5.0, key="rr_p0"
        )
        d1_rr = st.number_input(
            "Dividendo esperado ($D_1$)", min_value=0.01, value=7.5, step=0.5, key="rr_d1"
        )
    ...
    with c2:
        ...
        with paso_a_paso():
            ...
            themed_info(
                f"<div style='{css_paso}'><span style='{math_style}'>k</span> = {k_calc * 100:.4f}%</div>"
            )
