...
st.markdown(
    '<div class="page-sub">Divergencia de precios y sensibilidades entre BSM, CRR, Heston y Merton bajo los mismos parámetros base. BSM es el benchmark de referencia.</div>',
    unsafe_allow_html=True,
)
...
with st.expander(" Parámetros comunes", expanded=True):
    ...
    with c3:
        sigma = (
            st.number_input(
                "σ — volatilidad base (%)", value=20.0, min_value=0.1, step=0.5, format="%.2f"
            )
            / 100
        )
...
with st.expander(" Parámetros de Heston y Merton", expanded=False):
    ...
    with ch:
        st.markdown(
            f"<div style='font-size:0.72rem;color:{text_sub};margin-bottom:0.4rem'>Heston</div>",
            unsafe_allow_html=True,
        )
        h_kappa = st.slider("κ (reversión)", 0.1, 10.0, 2.0, 0.1, key="h_kappa")
        h_theta = (
            st.slider("θ̄ vol largo plazo (%)", 1.0, 80.0, 20.0, 0.5, key="h_theta") / 100
        ) ** 2
        h_xi = st.slider("ξ (vol de vol)", 0.05, 1.5, 0.3, 0.05, key="h_xi")
        h_rho = st.slider("ρ (correlación)", -0.99, 0.99, -0.7, 0.01, key="h_rho")
    with cm:
        st.markdown(
            f"<div style='font-size:0.72rem;color:{text_sub};margin-bottom:0.4rem'>Merton</div>",
            unsafe_allow_html=True,
        )
...
with tab1:
    ...
    with st.spinner("Calculando series por strike..."):
        bsm_s = [
            BSMEngine(S, k, T, r, sigma, q).call_price()
            if oc == "call"
            else BSMEngine(S, k, T, r, sigma, q).put_price()
            for k in strikes
        ]
        crr_s = [
            CRREngine(S, k, T, r, sigma, q, 200).call_price()
            if oc == "call"
            else CRREngine(S, k, T, r, sigma, q, 200).put_price()
            for k in strikes
        ]
        heston_s = []
        merton_s = []
    ...
    fig.update_layout(
        **plotly_layout(f"Precios {opt_choice} por Strike — todos los modelos"), height=400
    )
...
with tab2:
    ...
    with st.spinner("Sensibilidad a σ..."):
        ...
        heston_sig = [
            HestonEngine(S, K, T, r, q, s**2, h_kappa, s**2, h_xi, h_rho).call_price()
            for s in sigmas
        ]
        merton_sig = [
            MertonEngine(S, K, T, r, s, q, m_lam, m_mu_j, m_sigma_j).call_price() for s in sigmas
        ]
    ...
    for vals, model in zip([bsm_sig, crr_sig, heston_sig, merton_sig], MODEL_COLORS):
        fig2.add_trace(
            go.Scatter(
                x=sigmas * 100, y=vals, name=model, line=dict(color=MODEL_COLORS[model], width=2)
            )
        )
...
with tab3:
    st.markdown(
        f"<div style='font-size:0.8rem;color:{text_sub};margin-bottom:0.8rem'>Diferencia porcentual de cada modelo vs BSM a través de una grilla de σ × T.</div>",
        unsafe_allow_html=True,
    )
        ...
        for i, t_h in enumerate(mats_h):
            for j, s_h in enumerate(sigmas_h):
                ...
                if model_heat == "CRR":
                    ...
                    other = HestonEngine(
                        S, K, t_h, r, q, s_h**2, h_kappa, s_h**2, h_xi, h_rho
                    ).call_price()
                else:
                    other = MertonEngine(
                        S, K, t_h, r, s_h, q, m_lam, m_mu_j, m_sigma_j
                    ).call_price()
    ...
    fig_h.update_layout(
        **plotly_layout(f"Divergencia {model_heat} vs BSM (%) — grilla σ × T"), height=420
    )
