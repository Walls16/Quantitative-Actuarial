...
with tab_crr:
    ...
    with c1:
        ...
        S_crr = st.number_input(
            "Precio Spot ($S_0$)", min_value=0.01, value=100.0, step=1.0, key="crr_S"
        )
        K_crr = st.number_input(
            "Precio de Ejercicio ($K$)", min_value=0.01, value=100.0, step=1.0, key="crr_K"
        )
        sig_crr = (
            st.number_input(
                "Volatilidad ($\\sigma$) %", min_value=0.01, value=20.0, step=0.5, key="crr_sig"
            )
            / 100
        )
        q_crr = (
            st.number_input("Dividendo continuo ($q$) %", value=0.0, step=0.1, key="crr_q") / 100
        )

    with c2:
        st.markdown("**Condiciones del contrato**")
        r_crr = (
            st.number_input("Tasa libre de riesgo ($r$) %", value=5.0, step=0.1, key="crr_r") / 100
        )
        T_crr = st.number_input(
            "Tiempo al vencimiento ($T$) años", min_value=0.01, value=1.0, step=0.25, key="crr_T"
        )
        N_crr = st.number_input(
            "Número de pasos ($N$)", min_value=1, max_value=500, value=4, step=1, key="crr_N"
        )

    with c3:
        st.markdown("**Tipo de opción**")
        tipo_op_crr = st.radio("Opción:", ["Call", "Put"], horizontal=True, key="crr_tipo")
        estilo_crr = st.radio(
            "Estilo:", ["Europea", "Americana"], horizontal=True, key="crr_estilo"
        )
    ...
    with paso_a_paso():
        ...
        st.latex(
            rf"u = e^{{{sig_crr:.4f} \times \sqrt{{{dt_crr:.6f}}}}} = e^{{{sig_crr * np.sqrt(dt_crr):.6f}}} = {u_crr:.6f}"
        )
        ...
        st.latex(
            rf"a = e^{{({r_crr:.4f} - {q_crr:.4f}) \times {dt_crr:.6f}}} = e^{{(r_crr-q_crr)*dt_crr:.6f}} = {a_crr:.6f}"
        )
        st.write("---")

        st.latex(r"p = \frac{a - d}{u - d}")
        st.latex(
            rf"p = \frac{{{a_crr:.6f} - {d_crr:.6f}}}{{{u_crr:.6f} - {d_crr:.6f}}} = \frac{{{a_crr - d_crr:.6f}}}{{{u_crr - d_crr:.6f}}} = {p_crr:.6f}"
        )

        if es_call_crr:
            themed_success(
                f"<div style='{css_paso}'><span style='{math_style}'>c</span> = ${precio_crr:,.4f}</div>"
            )
        else:
            themed_error(
                f"<div style='{css_paso}'><span style='{math_style}'>p</span> = ${precio_crr:,.4f}</div>"
            )
        ...
        def _renderizar_arbol(matriz, titulo, fmt="$.4f"):
            ...
            for col in range(n + 1):
                for row in range(col + 1):
                    ...
                    node_x.append(x_pos)
                    node_y.append(y_pos)
                    node_text.append(format(val, ".4f") if fmt == "$.4f" else f"${val:,.4f}")
    ...
    with st.expander("Convergencia del Árbol Binomial a Black-Scholes"):
        ...
        if not es_amer_crr:
            ...
            bsm_ref = quact.black_scholes(S_crr, K_crr, r_crr, sig_crr, T_crr, es_call_crr, q_crr)
...
with tab_bsm:
    ...
    with c1:
        ...
        S_bsm = st.number_input(
            "Precio Spot / Subyacente ($S_0$)", min_value=0.01, value=100.0, step=1.0, key="bsm_S"
        )
        K_bsm = st.number_input(
            "Precio de Ejercicio ($K$)", min_value=0.01, value=100.0, step=1.0, key="bsm_K"
        )
        r_bsm = (
            st.number_input("Tasa libre de riesgo ($r$) %", value=5.0, step=0.1, key="bsm_r") / 100
        )
        sig_bsm = (
            st.number_input(
                "Volatilidad ($\\sigma$) %", min_value=0.01, value=20.0, step=0.5, key="bsm_sig"
            )
            / 100
        )

        if variante_bsm != "Opción Perpetua (T → ∞)":
            T_bsm = st.number_input(
        ...
        tipo_bsm = st.radio("Tipo:", ["Call", "Put"], horizontal=True, key="bsm_tipo")
    ...
    with c1:
        ...
        if variante_bsm == "BSM Estándar (sin dividendos)":
            ...
            d1_v = (np.log(S_bsm / K_bsm) + (r_bsm + sig_bsm**2 / 2) * T_bsm) / (
                sig_bsm * np.sqrt(T_bsm)
            )
            d2_v = d1_v - sig_bsm * np.sqrt(T_bsm)

        elif variante_bsm == "BSM con dividendo continuo (Merton, 1973)":
            q_bsm = (
                st.number_input("Dividendo continuo ($q$) %", value=2.0, step=0.1, key="bsm_q")
                / 100
            )
            prima_bsm = quact.black_scholes(S_bsm, K_bsm, r_bsm, sig_bsm, T_bsm, es_call_bsm, q_bsm)
            formula_c = r"c = S_0 e^{-qT} N(d_1) - K e^{-rT} N(d_2)"
            formula_p = r"p = K e^{-rT} N(-d_2) - S_0 e^{-qT} N(-d_1)"
            formula_d = r"d_1 = \frac{\ln(S_0/K) + (r - q + \sigma^2/2)T}{\sigma\sqrt{T}}"
            d1_v = (np.log(S_bsm / K_bsm) + (r_bsm - q_bsm + sig_bsm**2 / 2) * T_bsm) / (
                sig_bsm * np.sqrt(T_bsm)
            )
            d2_v = d1_v - sig_bsm * np.sqrt(T_bsm)

        elif variante_bsm == "BSM para Futuros (Black, 1976)":
            F_bsm = st.number_input(
                "Precio del Futuro ($F_0$)", min_value=0.01, value=100.0, step=1.0, key="bsm_F"
            )
            ...
            formula_d = r"d_1 = \frac{\ln(F_0/K) + (\sigma^2/2) T}{\sigma\sqrt{T}}"
            d1_v = (np.log(F_bsm / K_bsm) + sig_bsm**2 * T_bsm / 2) / (sig_bsm * np.sqrt(T_bsm))
            d2_v = d1_v - sig_bsm * np.sqrt(T_bsm)

        elif variante_bsm == "BSM para Divisas (Garman-Kohlhagen, 1983)":
            rf_bsm = (
                st.number_input("Tasa extranjera ($r_f$) %", value=3.0, step=0.1, key="bsm_rf")
                / 100
            )
            prima_bsm = quact.black_scholes(
                S_bsm, K_bsm, r_bsm, sig_bsm, T_bsm, es_call_bsm, rf_bsm
            )
            formula_c = r"c = S_0 e^{-r_f T} N(d_1) - K e^{-r_d T} N(d_2)"
            formula_p = r"p = K e^{-r_d T} N(-d_2) - S_0 e^{-r_f T} N(-d_1)"
            formula_d = r"d_1 = \frac{\ln(S_0/K) + (r_d - r_f + \sigma^2/2)T}{\sigma\sqrt{T}}"
            d1_v = (np.log(S_bsm / K_bsm) + (r_bsm - rf_bsm + sig_bsm**2 / 2) * T_bsm) / (
                sig_bsm * np.sqrt(T_bsm)
            )
            d2_v = d1_v - sig_bsm * np.sqrt(T_bsm)

        elif variante_bsm == "BSM con dividendos discretos":
            n_div_bsm = st.number_input(
                "Número de dividendos", min_value=1, max_value=6, value=1, step=1, key="bsm_ndiv"
            )
            divs_bsm = []
            ...
            d1_v = (np.log(S_adj / K_bsm) + (r_bsm + sig_bsm**2 / 2) * T_bsm) / (
                sig_bsm * np.sqrt(T_bsm)
            )
            d2_v = d1_v - sig_bsm * np.sqrt(T_bsm)
    ...
    with c2:
        if prima_bsm is not None:
            ...
            if T_bsm is not None:
                separador()
                with st.expander(
                    "Griegas (BSM) — Sensibilidades del contrato actual", expanded=False
                ):
                    ...
                    gcol1.metric(
                        "Delta", f"{_gr['delta']:+.5f}", help="+$1 en subyacente → cambio en prima"
                    )
                    gcol2.metric(
                        "Gamma",
                        f"{_gr['gamma']:.5f}",
                    ...
                    gcol4.metric(
                        "Vega", f"{_gr['vega']:.5f}", help="Cambio ante +1% absoluto en volatilidad"
                    )
                    gcol5.metric(
                        "Rho",
                        f"{_gr['rho']:+.5f}",
    ...
    if prima_bsm is not None and variante_bsm != "Opción Perpetua (T → ∞)":
        ...
        with paso_a_paso():
            ...
            if "Estándar" in variante_bsm:
                st.latex(
                    rf"d_1 = \frac{{\ln({S_bsm:.2f}/{K_bsm:.2f}) + ({r_bsm:.4f} + \frac{{{sig_bsm:.4f}^2}}{{2}}){T_bsm:.4f}}}{{{sig_bsm:.4f}\sqrt{{{T_bsm:.4f}}}}}"
                )
                st.latex(
                    rf"d_1 = \frac{{{np.log(S_bsm / K_bsm):.6f} + {(r_bsm + sig_bsm**2 / 2) * T_bsm:.6f}}}{{{sig_bsm * np.sqrt(T_bsm):.6f}}} = {d1_v:.6f}"
                )
                t1 = S_bsm
                ...
                st.latex(
                    rf"d_1 = \frac{{\ln({S_bsm:.2f}/{K_bsm:.2f}) + ({r_bsm:.4f} - {q_bsm:.4f} + \frac{{{sig_bsm:.4f}^2}}{{2}}){T_bsm:.4f}}}{{{sig_bsm:.4f}\sqrt{{{T_bsm:.4f}}}}}"
                )
                st.latex(
                    rf"d_1 = \frac{{{np.log(S_bsm / K_bsm):.6f} + {(r_bsm - q_bsm + sig_bsm**2 / 2) * T_bsm:.6f}}}{{{sig_bsm * np.sqrt(T_bsm):.6f}}} = {d1_v:.6f}"
                )
                ...
                st.latex(
                    rf"d_1 = \frac{{\ln({F_bsm:.2f}/{K_bsm:.2f}) + (\frac{{{sig_bsm:.4f}^2}}{{2}}){T_bsm:.4f}}}{{{sig_bsm:.4f}\sqrt{{{T_bsm:.4f}}}}}"
                )
                st.latex(
                    rf"d_1 = \frac{{{np.log(F_bsm / K_bsm):.6f} + {(sig_bsm**2 / 2) * T_bsm:.6f}}}{{{sig_bsm * np.sqrt(T_bsm):.6f}}} = {d1_v:.6f}"
                )
                ...
                st.latex(
                    rf"d_1 = \frac{{\ln({S_bsm:.2f}/{K_bsm:.2f}) + ({r_bsm:.4f} - {rf_bsm:.4f} + \frac{{{sig_bsm:.4f}^2}}{{2}}){T_bsm:.4f}}}{{{sig_bsm:.4f}\sqrt{{{T_bsm:.4f}}}}}"
                )
                st.latex(
                    rf"d_1 = \frac{{{np.log(S_bsm / K_bsm):.6f} + {(r_bsm - rf_bsm + sig_bsm**2 / 2) * T_bsm:.6f}}}{{{sig_bsm * np.sqrt(T_bsm):.6f}}} = {d1_v:.6f}"
                )
                ...
                st.latex(
                    rf"d_1 = \frac{{\ln({S_adj:.6f}/{K_bsm:.2f}) + ({r_bsm:.4f} + \frac{{{sig_bsm:.4f}^2}}{{2}}){T_bsm:.4f}}}{{{sig_bsm:.4f}\sqrt{{{T_bsm:.4f}}}}}"
                )
                st.latex(
                    rf"d_1 = \frac{{{np.log(S_adj / K_bsm):.6f} + {(r_bsm + sig_bsm**2 / 2) * T_bsm:.6f}}}{{{sig_bsm * np.sqrt(T_bsm):.6f}}} = {d1_v:.6f}"
                )
                t1 = S_adj
                t2 = K_bsm * np.exp(-r_bsm * T_bsm)

            st.latex(
                rf"d_2 = d_1 - \sigma\sqrt{{T}} = {d1_v:.6f} - {sig_bsm * np.sqrt(T_bsm):.6f} = {d2_v:.6f}"
            )
            st.write("---")
            ...
            if es_call_bsm:
                ...
                if "Estándar" in variante_bsm or "discretos" in variante_bsm:
                    st.latex(
                        rf"c = {t1:.4f}({Nd1:.6f}) - {K_bsm:.2f} e^{{-{r_bsm:.4f} \times {T_bsm:.4f}}}({Nd2:.6f})"
                    )
                elif "Futuros" in variante_bsm:
                    st.latex(
                        rf"c = e^{{-{r_bsm:.4f} \times {T_bsm:.4f}}}[{F_bsm:.2f}({Nd1:.6f}) - {K_bsm:.2f}({Nd2:.6f})]"
                    )
                elif "Divisas" in variante_bsm:
                    st.latex(
                        rf"c = {S_bsm:.2f} e^{{-{rf_bsm:.4f} \times {T_bsm:.4f}}}({Nd1:.6f}) - {K_bsm:.2f} e^{{-{r_bsm:.4f} \times {T_bsm:.4f}}}({Nd2:.6f})"
                    )
                elif "continuo" in variante_bsm:
                    st.latex(
                        rf"c = {S_bsm:.2f} e^{{-{q_bsm:.4f} \times {T_bsm:.4f}}}({Nd1:.6f}) - {K_bsm:.2f} e^{{-{r_bsm:.4f} \times {T_bsm:.4f}}}({Nd2:.6f})"
                    )

                if "Futuros" in variante_bsm:
                    st.latex(
                        rf"c = {np.exp(-r_bsm * T_bsm):.6f} [{F_bsm * Nd1:.6f} - {K_bsm * Nd2:.6f}] = {prima_bsm:.4f}"
                    )
                else:
                ...
                if "Estándar" in variante_bsm or "discretos" in variante_bsm:
                    st.latex(
                        rf"p = {K_bsm:.2f} e^{{-{r_bsm:.4f} \times {T_bsm:.4f}}}({nNd2:.6f}) - {t1:.4f}({nNd1:.6f})"
                    )
                elif "Futuros" in variante_bsm:
                    st.latex(
                        rf"p = e^{{-{r_bsm:.4f} \times {T_bsm:.4f}}}[{K_bsm:.2f}({nNd2:.6f}) - {F_bsm:.2f}({nNd1:.6f})]"
                    )
                elif "Divisas" in variante_bsm:
                    st.latex(
                        rf"p = {K_bsm:.2f} e^{{-{r_bsm:.4f} \times {T_bsm:.4f}}}({nNd2:.6f}) - {S_bsm:.2f} e^{{-{rf_bsm:.4f} \times {T_bsm:.4f}}}({nNd1:.6f})"
                    )
                elif "continuo" in variante_bsm:
                    st.latex(
                        rf"p = {K_bsm:.2f} e^{{-{r_bsm:.4f} \times {T_bsm:.4f}}}({nNd2:.6f}) - {S_bsm:.2f} e^{{-{q_bsm:.4f} \times {T_bsm:.4f}}}({nNd1:.6f})"
                    )

                if "Futuros" in variante_bsm:
                    st.latex(
                        rf"p = {np.exp(-r_bsm * T_bsm):.6f} [{K_bsm * nNd2:.6f} - {F_bsm * nNd1:.6f}] = {prima_bsm:.4f}"
                    )
                else:
            ...
            if es_call_bsm:
                themed_success(
                    f"<div style='{css_paso}'><span style='{math_style}'>c</span> = ${prima_bsm:,.4f}</div>"
                )
            else:
                themed_error(
                    f"<div style='{css_paso}'><span style='{math_style}'>p</span> = ${prima_bsm:,.4f}</div>"
                )
        ...
        with paso_a_paso():
            st.latex(formula_d)
            st.latex(
                rf"h = \frac{{1}}{{2}} + \sqrt{{\frac{{1}}{{4}} + \frac{{2({r_bsm:.4f})}}{{{sig_bsm:.4f}^2}}}}"
            )
            ...
            if es_call_bsm:
                st.latex(formula_c)
                st.latex(
                    rf"C^* = \frac{{{K_bsm:.2f}}}{{{h_val:.6f}-1}}\left(\frac{{({h_val:.6f}-1){S_bsm:.2f}}}{{{h_val:.6f}({K_bsm:.2f})}}\right)^{{{h_val:.6f}}}"
                )
                st.latex(
                    rf"C^* = {K_bsm / (h_val - 1):.6f} \left({(h_val - 1) * S_bsm / (h_val * K_bsm):.6f}\right)^{{{h_val:.6f}}} = {prima_bsm:.4f}"
                )
                themed_success(
                    f"<div style='{css_paso}'><span style='{math_style}'>C<sup>*</sup></span> = ${prima_bsm:,.4f}</div>"
                )
            else:
                st.latex(r"h^* = \frac{1}{2} - \sqrt{\frac{1}{4} + \frac{2r}{\sigma^2}}")
                h_star = 0.5 - np.sqrt(0.25 + 2 * r_bsm / (sig_bsm**2))
                st.latex(
                    rf"h^* = 0.5 - \sqrt{{0.25 + {2 * r_bsm / (sig_bsm**2):.6f}}} = {h_star:.6f}"
                )
                st.latex(formula_p)
                st.latex(
                    rf"P^* = \frac{{{K_bsm:.2f}}}{{1 - ({h_star:.6f})}}\left(\frac{{(1 - ({h_star:.6f})){S_bsm:.2f}}}{{{h_star:.6f}({K_bsm:.2f})}}\right)^{{{h_star:.6f}}}"
                )
                st.latex(rf"P^* = {prima_bsm:.4f}")
                themed_error(
                    f"<div style='{css_paso}'><span style='{math_style}'>P<sup>*</sup></span> = ${prima_bsm:,.4f}</div>"
                )
...
with tab_griegas:
    ...
    with c_in1:
        ...
        S_gr = st.number_input(
            "Precio Spot ($S_0$)", min_value=0.01, value=100.0, step=1.0, key="gr_S"
        )
    ...
    with c_in2:
        ...
        r_gr = (
            st.number_input("Tasa libre de riesgo ($r$) %", value=5.0, step=0.1, key="gr_r") / 100
        )
        sig_gr = (
            st.number_input(
                "Volatilidad ($\\sigma$) %", min_value=0.01, value=20.0, step=0.5, key="gr_sig"
            )
            / 100
        )
        T_gr = st.number_input(
            "Tiempo al vencimiento ($T$) años", min_value=0.001, value=1.0, step=0.25, key="gr_T"
        )
    ...
    with c_in3:
        ...
        col_m1.metric(
            "Δ Delta", f"{griegas['delta']:+.5f}", help="+$1 en subyacente → cambio en prima"
        )
        col_m2.metric(
            "Γ Gamma", f"{griegas['gamma']:.5f}", help="Cambio en Delta ante +$1 en subyacente"
        )
        col_m1.metric(
            "Θ Theta /día", f"{griegas['theta']:+.5f}", help="Pérdida de valor por cada día natural"
        )
        col_m2.metric(
            "𝒱 Vega /1%σ", f"{griegas['vega']:.5f}", help="Cambio ante +1% absoluto en volatilidad"
        )
        col_m1.metric(
            "ρ Rho /1%r",
            f"{griegas['rho']:+.5f}",
    ...
    with col_g:
        ...
        fig.add_trace(
            go.Scatter(
                ...
                mode="lines",
                name="Γ Gamma",
                line=dict(color=COL["gamma"], width=2.5),
            )
        )
        _vline(fig)
    ...
    with col_t:
        ...
        fig.add_trace(
            go.Scatter(
                ...
                mode="lines",
                name="Θ Theta/día",
                line=dict(color=COL["theta"], width=2.5),
            )
        )
    ...
    with col_v:
        ...
        fig.add_trace(
            go.Scatter(
                ...
                mode="lines",
                name="𝒱 Vega/1%σ",
                line=dict(color=COL["vega"], width=2.5),
            )
        )
        _vline(fig)
    ...
    with col_r:
        ...
        fig.add_trace(
            go.Scatter(
                ...
                mode="lines",
                name="ρ Rho/1%r",
                line=dict(color=COL["rho"], width=2.5),
            )
        )
        fig.add_hline(y=0, line_dash="dash", line_color=c["text_muted"], line_width=1)
        _vline(fig)
        ...
        fig3.update_layout(
            ...
            scene=dict(
                xaxis=dict(
                    title="Spot S",
                    tickfont=dict(color=c["text_muted"]),
                    gridcolor=plotly_color(c["border"]),
                    backgroundcolor=c["bg_light"],
                ),
                yaxis=dict(
                    title="T (años)",
                    tickfont=dict(color=c["text_muted"]),
    ...
    with tab_s3d_gamma:
        st.plotly_chart(
            _surf_fig(
                _Z_gamma, f"Gamma | K={K_gr:.0f} | σ={sig_gr * 100:.0f}%", "Gamma Γ", "Greens"
            ),
            use_container_width=True,
        )
        themed_info(
    ...
    with col_tv1:
        st.plotly_chart(
            _greek_vs_T("delta", "Δ Delta vs T | OTM · ATM · ITM", "Delta Δ"),
            use_container_width=True,
        )
        themed_info(
    ...
    with col_tv2:
        st.plotly_chart(
            _greek_vs_T("gamma", "Γ Gamma vs T | OTM · ATM · ITM", "Gamma Γ"),
            use_container_width=True,
        )
        themed_info(
    ...
    with col_tv3:
        st.plotly_chart(
            _greek_vs_T("theta", "Θ Theta vs T | OTM · ATM · ITM", "Theta Θ ($/día)"),
            use_container_width=True,
        )
        themed_info(
    ...
    with col_tv4:
        st.plotly_chart(
            _greek_vs_T("vega", "𝒱 Vega vs T | OTM · ATM · ITM", "Vega 𝒱 ($/1%σ)"),
            use_container_width=True,
        )
        themed_info(
    ...
    with st.expander("Griegas de Segundo Orden: Vanna · Vomma · Charm · Speed · Color"):
        ...
        with col_v1:
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    mode="lines",
                    name="Vanna (∂Δ/∂σ)",
                    line=dict(color=COL["vanna"], width=2.5),
                )
            )
            fig.add_hline(y=0, line_dash="dash", line_color=c["text_muted"], line_width=1)
            _vline(fig)
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    mode="lines",
                    name="Speed (∂Γ/∂S)",
                    line=dict(color=COL["speed"], width=2.5),
                )
            )
            fig.add_hline(y=0, line_dash="dash", line_color=c["text_muted"], line_width=1)
            _vline(fig)
        ...
        with col_v2:
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    mode="lines",
                    name="Charm (∂Δ/∂T)",
                    line=dict(color=COL["charm"], width=2.5),
                )
            )
            fig.add_hline(y=0, line_dash="dash", line_color=c["text_muted"], line_width=1)
            _vline(fig)
        ...
        fig.add_trace(
            go.Scatter(
                ...
                mode="lines",
                name="Color (∂Γ/∂T)",
                line=dict(color=COL["color_"], width=2.5),
            )
        )
        fig.add_hline(y=0, line_dash="dash", line_color=c["text_muted"], line_width=1)
        _vline(fig)
        fig = _layout(
            fig, "Color = ∂Γ/∂T (decaimiento de Gamma, por día)", "Color (Γ/día)", height=270
        )
        st.plotly_chart(fig, use_container_width=True)
        themed_info(
        ...
        with col_f2:
            st.latex(
                r"\text{Charm}_{call}=q e^{-qT}N(d_1)-e^{-qT}n(d_1)\frac{2(r-q)T-d_2\sigma\sqrt{T}}{2T\sigma\sqrt{T}}"
            )
            st.latex(
                r"\text{Color}=-\frac{e^{-qT}n(d_1)}{2ST\sigma\sqrt{T}}\left(2qT+1+\frac{2(r-q)T-d_2\sigma\sqrt{T}}{\sigma\sqrt{T}}d_1\right)"
            )
    ...
    with st.expander("Griegas vs Volatilidad implícita (σ) — Shocks de vol"):
        ...
        with col_sv1:
            st.plotly_chart(
                _greek_vs_sig("delta", "Δ Delta vs σ | OTM · ATM · ITM", "Delta Δ"),
                use_container_width=True,
            )
            st.plotly_chart(
                _greek_vs_sig("theta", "Θ Theta vs σ | OTM · ATM · ITM", "Theta Θ ($/día)"),
                use_container_width=True,
            )
        with col_sv2:
            st.plotly_chart(
                _greek_vs_sig("gamma", "Γ Gamma vs σ | OTM · ATM · ITM", "Gamma Γ"),
                use_container_width=True,
            )
            st.plotly_chart(
                _greek_vs_sig("vega", "𝒱 Vega vs σ | OTM · ATM · ITM", "Vega 𝒱 ($/1%σ)"),
                use_container_width=True,
            )
    ...
    with st.expander("Fórmulas matemáticas completas (1er y 2do orden, con dividendo q)"):
        ...
        with col_f1:
            ...
            st.latex(
                r"d_1=\frac{\ln(S/K)+(r-q+\frac{\sigma^2}{2})T}{\sigma\sqrt{T}},\quad d_2=d_1-\sigma\sqrt{T}"
            )
            st.latex(r"\Delta_{call}=e^{-qT}N(d_1),\quad \Delta_{put}=-e^{-qT}N(-d_1)")
            st.latex(r"\Gamma=\frac{e^{-qT}n(d_1)}{S\sigma\sqrt{T}}")
            st.latex(
                r"\Theta_{call}=-\frac{Sn(d_1)\sigma e^{-qT}}{2\sqrt{T}}-rKe^{-rT}N(d_2)+qSe^{-qT}N(d_1)"
            )
        ...
        with col_f2:
            ...
            st.latex(r"\text{Vomma}=\mathcal{V}\cdot\frac{d_1 d_2}{\sigma}")
            st.latex(
                r"\text{Charm}_{call}=qe^{-qT}N(d_1)-e^{-qT}n(d_1)\frac{2(r-q)T-d_2\sigma\sqrt{T}}{2T\sigma\sqrt{T}}"
            )
            st.latex(r"\text{Speed}=-\frac{\Gamma}{S}\left(\frac{d_1}{\sigma\sqrt{T}}+1\right)")
            st.latex(
                r"\text{Color}=-\frac{e^{-qT}n(d_1)}{2ST\sigma\sqrt{T}}\left(2qT+1+d_1\frac{2(r-q)T-d_2\sigma\sqrt{T}}{\sigma\sqrt{T}}\right)"
            )
...
with tab_comp:
    ...
    c1, c2, c3 = st.columns(3)
    with c1:
        S_cp = st.number_input(
            "Precio Spot ($S_0$)", min_value=0.01, value=100.0, step=1.0, key="cp_S"
        )
        K_cp = st.number_input("Strike ($K$)", min_value=0.01, value=105.0, step=1.0, key="cp_K")
    with c2:
        r_cp = (
            st.number_input("Tasa libre de riesgo ($r$) %", value=5.0, step=0.1, key="cp_r") / 100
        )
        sig_cp = (
            st.number_input(
                "Volatilidad ($\\sigma$) %", min_value=0.01, value=20.0, step=0.5, key="cp_sig"
            )
            / 100
        )
    with c3:
        T_cp = st.number_input(
            "Tiempo al vencimiento ($T$) años", min_value=0.01, value=1.0, step=0.25, key="cp_T"
        )
        q_cp = st.number_input("Dividendo continuo ($q$) %", value=0.0, step=0.1, key="cp_q") / 100
        tipo_cp = st.radio(
            "Tipo de Opción Europea:", ["Call", "Put"], horizontal=True, key="cp_tipo"
        )
    ...
    col_crr_m.metric(
        "Precio CRR (N=500 Pasos)",
        f"${precios_crr_comp[-1]:.4f}",
        delta=f"Margen de Error = ${abs(precios_crr_comp[-1] - bsm_comp):.6f}",
        delta_color="off",
    )
...
with tab_est:
    ...
    c1, c2 = st.columns(2)
    with c1:
        S_est = st.number_input(
            "Precio Spot actual ($S_0$)", min_value=0.01, value=100.0, step=1.0, key="est_S"
        )
        r_est = (
            st.number_input("Tasa libre de riesgo ($r$) %", value=5.0, step=0.1, key="est_r") / 100
        )
        sig_est = (
            st.number_input(
        ...
        T_est = st.number_input(
            "Tiempo al vencimiento ($T$) años", min_value=0.01, value=1.0, step=0.25, key="est_T"
        )
        q_est = (
            st.number_input("Dividendo continuo ($q$) %", value=0.0, step=0.1, key="est_q") / 100
        )
    ...
    with c2:
        ...
        if estrategia_sel == "Manual (configura tú mismo)":
            ...
            for i in range(int(n_patas)):
                ...
                cols_p = st.columns(4)
                tipo_p = cols_p[0].selectbox("Opción", ["call", "put"], key=f"est_tipo_{i}")
                pos_p = cols_p[1].selectbox(
                    "Postura",
                    [1, -1],
                    format_func=lambda x: "Long (Compra)" if x == 1 else "Short (Venta)",
                    key=f"est_pos_{i}",
                )
                K_p = cols_p[2].number_input(
                    "Strike ($K$)", min_value=0.01, value=float(S_est), step=1.0, key=f"est_K_{i}"
                )
                prima_p = cols_p[3].number_input(
                    "Prima ($)",
                    min_value=0.0,
            ...
            K1_e = st.number_input(
                "Strike de soporte ($K_1$)",
                min_value=0.01,
                value=float(S_est) * 0.95,
                step=1.0,
                key="est_K1",
            )
            K2_e = st.number_input(
                "Strike de resistencia ($K_2$)",
                min_value=0.01,
                value=float(S_est) * 1.05,
                step=1.0,
                key="est_K2",
            )

            c_K1 = _prima(K1_e, True)
            p_K1 = _prima(K1_e, False)
            c_K2 = _prima(K2_e, True)
            p_K2 = _prima(K2_e, False)
            ...
            if estrategia_sel == "Bull Call Spread":
                ...
                themed_info(
                    "**Long Call** K1 + **Short Call** K2  →  Limita el costo inicial a cambio de topar la ganancia. Beneficio direccional si el activo sube moderadamente."
                )
                ...
                themed_info(
                    "**Long Put** K2 + **Short Put** K1  →  Beneficio direccional si el activo baja moderadamente, financiando el costo del Put con la venta del strike inferior."
                )

            elif estrategia_sel == "Long Straddle":
                K_mid = (K1_e + K2_e) / 2
                c_mid = _prima(K_mid, True)
                p_mid = _prima(K_mid, False)
                patas = [
                    {"tipo": "call", "posicion": 1, "K": K_mid, "prima": c_mid},
                    {"tipo": "put", "posicion": 1, "K": K_mid, "prima": p_mid},
                ]
                themed_info(
                    "**Long Call + Long Put** (mismo strike)  →  Estrategia pura de volatilidad. Genera beneficios si ocurre un movimiento brusco, sin importar la dirección."
                )
                ...
                themed_info(
                    "**Short Call + Short Put** (mismo strike)  →  Estrategia de ingreso pasivo. Apuesta a que el precio se quedará estancado cobrando el decaimiento temporal (Theta)."
                )
                ...
                themed_info(
                    "**Long Call** K2 + **Long Put** K1  →  Más barato de estructurar que el Straddle, pero requiere una explosión de volatilidad mucho mayor para romper los puntos de equilibrio."
                )
                ...
                themed_info(
                    "**Long 1 Call K1 + Short 2 Calls K_mid + Long 1 Call K2** →  Estrategia de bajo costo que busca capturar el máximo beneficio exactamente en el strike central."
                )
                ...
                themed_info(
                    "**Short Call** K2 (vendido contra acciones que ya posees)  →  Genera un flujo de efectivo extra constante a cambio de comprometerte a vender si el precio se dispara."
                )
                themed_warning(
                    "Por simplicidad visual, la pata subyacente (acción en cartera) no se diagrama en el perfil (se asume costo hundido a <span style='font-family: serif; font-style: italic;'>S<sub>0</sub></span>)."
                )
                ...
                themed_info(
                    "**Long Put** K1 (comprado para asegurar un activo)  →  Opera como una póliza de seguro, limitando la pérdida máxima del portafolio."
                )
                themed_warning(
                    "Por simplicidad visual, la pata subyacente asegurada no se diagrama en el perfil."
                )
                ...
                themed_info(
                    "**Long Call** K2 + **Short Put** K1  →  Estructura apalancada. Fija una postura direccional alcista financiando parcial o totalmente el costo del Call."
                )
    ...
    if costo_total < 0:
        themed_success(
            f"**Prima neta recibida:** ${abs(costo_total):,.4f}  (La estructura genera un ingreso inicial de efectivo al mercado)"
        )
    else:
        themed_error(
            f"**Prima neta pagada:** ${costo_total:,.4f}  (La estructura requiere un desembolso inicial de capital)"
        )
    ...
    with st.expander("Desglose Financiero (Límites y Puntos de Equilibrio)"):
        ...
        col_k2.metric(
            "Riesgo (Pérdida Máxima)",
            f"${abs(perdida_max):,.4f}" if perdida_max > -1e8 else "Ilimitada",
        )
        if breakevens:
            col_k3.metric(
                "Breakeven (Puntos Muertos)", " | ".join([f"${be:.2f}" for be in breakevens])
            )
        else:
...
with tab_real:
    ...
    with col_t2:
        ...
        btn_buscar_real = st.button(
            "Descargar Historial de Yahoo Finance", use_container_width=True, key="btn_real"
        )
    with col_t3:
    ...
    with col_p1:
        ...
        tipo_real = st.radio(
            "Clase de Opción:",
            ["Call (Derecho de compra)", "Put (Derecho de venta)"],
            horizontal=True,
            key="real_tipo",
        )
        estilo_real = st.radio(
            "Ejercicio:",
            ["Europea (A término)", "Americana (Flexible)"],
            horizontal=True,
            key="real_estilo",
        )
    ...
    with col_r1:
        ...
        if es_call_real:
            themed_success(
                f"<h4 style='margin:0; color:inherit;'>Límite Continuo BSM: ${prima_bsm_real:,.4f}</h4>"
            )
            themed_success(
                f"<h4 style='margin:0; color:inherit;'>Modelo Discreto CRR: ${prima_crr_real:,.4f}</h4>"
            )
        else:
            themed_error(
                f"<h4 style='margin:0; color:inherit;'>Límite Continuo BSM: ${prima_bsm_real:,.4f}</h4>"
        ...
        diff_pct = (
            abs(prima_bsm_real - prima_crr_real) / prima_bsm_real * 100 if prima_bsm_real > 0 else 0
        )
        st.metric(
            "Margen de Discrepancia BSM vs CRR",
            f"${abs(prima_bsm_real - prima_crr_real):,.4f}",
    ...
    with col_r2:
        ...
        g1.metric(
            "Δ Delta",
            f"{griegas_real['delta']:+.5f}",
            help="+$1 en el subyacente → cambio en prima",
        )
        g2.metric("Γ Gamma", f"{griegas_real['gamma']:.5f}", help="Cambio en Delta ante +$1")
        g3.metric("Θ Theta", f"{griegas_real['theta']:+.5f}", help="Pérdida de valor por día")
        g4, g5, _ = st.columns(3)
        g4.metric("𝒱 Vega", f"{griegas_real['vega']:.5f}", help="Cambio ante +1% volatilidad")
        g5.metric("ρ Rho", f"{griegas_real['rho']:+.5f}", help="Cambio ante +1% en tasa r")
    ...
    fig_pay.add_hline(
        y=0, line_dash="dash", line_color=plotly_color(c_theme["border"]), line_width=1
    )
    fig_pay.add_vline(
        x=K_real,
        line_dash="dot",
        line_color=c_theme["primary"],
        annotation_text=f"K = {K_real:.2f}",
        annotation_position="top right",
    )
    fig_pay.add_vline(
        x=S_real,
        line_dash="solid",
        line_color=c_theme["success"],
        annotation_text=f"S₀ = {S_real:.2f}",
    ...
    tab_sens_vol, tab_sens_t = st.tabs(
        ["Impacto por Choque de Volatilidad", "Impacto por Decaimiento Temporal"]
    )
...
with tab_vol:
    ...
    with col_v1:
        ...
        S_vol = st.number_input(
            "Precio Spot de la Acción ($S_0$)",
            min_value=0.01,
            value=st.session_state.get("vol_S_key", 100.0),
            step=1.0,
            key="vol_S",
        )
        K_vol = st.number_input(
            "Strike Pactado ($K$)",
            min_value=0.01,
            value=st.session_state.get("vol_K_key", 100.0),
            step=1.0,
            key="vol_K_man",
        )
        precio_mkt = st.number_input(
            "Prima Negociada en Bolsa ($)",
            min_value=0.001,
            value=st.session_state.get("vol_precio_key", 10.0),
            step=0.5,
            key="vol_precio",
        )
    ...
    with col_v2:
        ...
        r_vol = (
            st.number_input(
                "Tasa libre de riesgo continua ($r$) %", value=5.0, step=0.1, key="vol_r"
            )
            / 100
        )
        q_vol = (
            st.number_input(
                "Carga por dividendo continuo ($q$) %", value=0.0, step=0.1, key="vol_q"
            )
            / 100
        )
        T_vol = st.number_input(
            "Años al expiración del derecho ($T$)",
            min_value=0.01,
            value=1.0,
            step=0.25,
            key="vol_T",
    ...
    with col_v3:
        ...
        ticker_vol = (
            st.text_input("Ingresar Ticker Global:", value="AAPL", key="vol_tick").strip().upper()
        )
        btn_vol_yf = st.button("Sincronizar Datos Teóricos", key="btn_vol_yf")
        if btn_vol_yf:
            with st.spinner(
                "Estableciendo enlace de red y calculando factor de prima de riesgo..."
            ):
                ...
                if sv is not None:
                    ...
                    precio_mercado_simulado = quact.black_scholes(
                        sv, sv, r_vol, vv + 0.05, T_vol, es_call_vol, q_vol
                    )
    ...
    try:
        sig_impl = implied_volatility_bsm(
            precio_mkt, S_vol, K_vol, r_vol, T_vol, es_call_vol, q_vol
        )
        ...
        with paso_a_paso():
            ...
            st.latex(
                rf"f(\sigma_{{impl}}) = \text{{BSM}}({S_vol:,.2f}, {K_vol:,.2f}, {r_vol:.4f}, \sigma_{{impl}}, {T_vol:.4f}) - {precio_mkt:,.4f} = 0"
            )
            alerta_metodo_numerico()
            st.latex(rf"\sigma_{{impl}} \approx {sig_impl:.6f}")
            themed_success(
                f"<div style='{css_paso}'><span style='{math_style}'>&sigma;<sub>impl</sub></span> = {sig_impl * 100:.4f}%</div>"
            )
    ...
    with tab_surf_real:
        ...
        sig_actual_surf = (
            sig_atm
            + _a_skew * np.log(m_actual) ** 2
            - _b_skew * np.log(m_actual)
            + _c_term / np.sqrt(T_vol)
        )
        fig_surf_real.add_scatter3d(
            x=[m_actual],
            y=[T_vol],
            z=[sig_actual_surf * 100],
            mode="markers",
            name=f"Contrato actual (M={m_actual:.2f})",
            marker=dict(size=7, color=c_surf["accent"], symbol="circle"),
        )
        fig_surf_real = _surf_vol_layout(
            fig_surf_real, f"Superficie de Volatilidad con Smile | σ_ATM={sig_atm * 100:.1f}%"
        )
    ...
    with tab_surf_diff:
        ...
        fig_surf_diff.add_trace(
            go.Scatter3d(
                x=_M_surf,
                y=[_T_surf_vol[0]] * len(_M_surf),
                z=[0.0] * len(_M_surf),
                mode="lines",
                name="Nivel BSM (Δσ=0)",
        ...
        fig_surf_diff = _surf_vol_layout(
            fig_surf_diff,
