    ...
    if "Pago Fijo (R)  →  conozco el Préstamo (VP)" in modo:
        ...
        if tipo_eng == "Monto fijo ($)":
            enganche = st.number_input(
                "Enganche ($)", min_value=0.0, value=50_000.0, step=5_000.0, key="am_eng_monto"
            )
        else:
        ...
        R_conocido = st.number_input(
            "Pago periódico fijo ($R$)", min_value=0.01, value=15_000.0, step=500.0, key="am_r"
        )
...
with c3:
    ...
    n_am = st.number_input(
        "Años del préstamo ($n$)", min_value=0.1, value=5.0, step=1.0, key="am_n"
    )
...
with c2:
    ...
    if tipo_tasa == "Tasa efectiva periódica":
        tasa_input = (
            st.number_input("Tasa efectiva periódica ($i_m$) %", value=1.5, step=0.1, key="am_ieff")
            / 100
        )
        str_tasa = r"i_m"
        ...
        tasa_input = (
            st.number_input("Tasa nominal anual ($i^{(m)}$) %", value=18.0, step=0.1, key="am_inom")
            / 100
        )
    ...
    with col_form:
        ...
        with paso_a_paso():
            ...
            if tasa_periodo > 0:
                ...
                st.latex(
                    rf"R = {vp_neto:,.2f} \left[ \frac{{{tasa_periodo:.6f}}}{{1 - {v_n:.6f}}} \right]"
                )
                st.latex(rf"R = {vp_neto:,.2f} ({factor:.6f}) = {pago_R:.2f}")

            themed_success(
                f"<div style='{css_paso}'><span style='{math_style}'>R</span> = ${pago_R:,.2f}</div>"
            )
    ...
    with col_form:
        ...
        with paso_a_paso():
            ...
            if tasa_periodo > 0:
                ...
                st.latex(
                    rf"VP = {R_conocido:,.2f} \left[ \frac{{1 - {v_n:.6f}}}{{{tasa_periodo:.6f}}} \right]"
                )
                st.latex(rf"VP = {R_conocido:,.2f} ({factor_vp:.6f}) = {vp_calc:.2f}")

            themed_info(
                f"<div style='{css_paso}'><span style='{math_style}'>VP</span> = ${vp_calc:,.2f}</div>"
            )
...
with tab_tabla:
    ...
    total_interes = (
        df_amort["Interés"].sum() if "Interés" in df_amort.columns else (pago_R * nm_am) - vp_final
    )
...
with tab_grafica:
    if (
        "Periodo" in df_amort.columns
        and "Amortización" in df_amort.columns
        and "Interés" in df_amort.columns
    ):
        fig = px.bar(
            df_amort,
            x="Periodo",
        ...
        themed_warning(
            "No se pudo generar la gráfica. Asegúrate de que tu motor devuelve las columnas: 'Periodo', 'Amortización' e 'Interés'."
        )
