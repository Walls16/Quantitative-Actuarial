...
st.markdown(
    '<div class="page-sub">Volatilidad estocástica con reversión a la media. Genera sonrisa de volatilidad de forma endógena, sin suponer σ constante.</div>',
    unsafe_allow_html=True,
)
...
with col_in:
    ...
    st.markdown(
        f"<div style='font-size:0.72rem;letter-spacing:0.14em;text-transform:uppercase;color:{text_sub};margin:1rem 0 0.5rem 0'>Parámetros de Heston</div>",
        unsafe_allow_html=True,
    )
    v0 = (
        st.number_input(
            "Varianza inicial (v₀ = σ₀²)",
            value=4.0,
            min_value=0.01,
            max_value=100.0,
            step=0.5,
        ...
        st.number_input(
            "σ₀ inicial (%)", value=20.0, min_value=1.0, max_value=100.0, step=0.5, format="%.2f"
        )
        / 100
    ) ** 2
    ...
    theta_pct = st.slider(
        "θ — vol largo plazo (%)", 1.0, 80.0, float(_p.get("theta_pct", 20.0)), 0.5
    )
...
tab1, tab2, tab3, tab4 = st.tabs(
    ["Sonrisa de volatilidad", "Superficie IV", "Sensibilidad a ρ y ξ", "Calibracion al mercado"]
)
...
with tab1:
    ...
    if show_multi_mat:
        ...
        fig.add_trace(
            go.Scatter(
                x=x_axis, y=ivs_heston * 100, name="Heston IV", line=dict(color=accent, width=2.5)
            )
        )
    fig.add_trace(
        go.Scatter(
...
with tab2:
    ...
    fig_surf.update_layout(
        ...
        title=dict(
            text="Superficie de Volatilidad Implícita — Heston", font=dict(color=text_main, size=14)
        ),
        height=500,
...
with tab3:
    ...
    with col_r:
        ...
        fig_r = go.Figure()
        fig_r.add_trace(
            go.Scatter(x=rhos, y=calls_rho, line=dict(color=accent, width=2), name="Call Heston")
        )
    ...
    with col_xi:
        ...
        fig_x = go.Figure()
        fig_x.add_trace(
            go.Scatter(x=xis, y=calls_xi, line=dict(color="#a78bfa", width=2), name="Call Heston")
        )
...
with tab4:
    ...
    st.markdown(
        f"<div style='font-size:0.72rem;color:{text_sub};text-transform:uppercase;letter-spacing:0.1em;margin-bottom:0.5rem'>Sonrisa de mercado (strike, IV%)</div>",
        unsafe_allow_html=True,
    )

    n_cal = st.slider("Numero de strikes de mercado", 3, 10, 6, key="hcal_n")
    default_ks = [
        S * m for m in [0.80, 0.90, 0.95, 1.00, 1.05, 1.10, 1.20, 1.30, 1.40, 1.50][:n_cal]
    ]
    ...
    for i in range(n_cal):
        col = cols_cal[i % 5]
        with col:
            k_c = col.number_input(
                f"K_{i + 1}",
                value=float(round(default_ks[i], 1)),
                step=1.0,
                format="%.1f",
                key=f"hcal_k{i}",
            )
            iv_c = col.number_input(
                f"IV_{i + 1} (%)",
                value=float(default_ivs[i] if i < len(default_ivs) else 20.0),
                min_value=0.5,
                max_value=150.0,
                step=0.1,
                format="%.2f",
    ...
    st.markdown(
        f"<div style='font-size:0.72rem;color:{text_sub};text-transform:uppercase;letter-spacing:0.1em;margin:0.8rem 0 0.4rem'>Punto inicial para la optimizacion</div>",
        unsafe_allow_html=True,
    )
    cc1, cc2, cc3, cc4, cc5 = st.columns(5)
    kappa0 = cc1.number_input(
        "kappa0", value=2.0, min_value=0.01, step=0.1, format="%.2f", key="hc_k0"
    )
    theta0 = (
        cc2.number_input(
            "theta0 (%)", value=20.0, min_value=0.1, step=0.5, format="%.1f", key="hc_t0"
        )
        / 100
    ) ** 2
    xi0 = cc3.number_input("xi0", value=0.3, min_value=0.01, step=0.05, format="%.3f", key="hc_xi0")
    rho0 = cc4.number_input(
        "rho0", value=-0.5, min_value=-0.99, max_value=0.99, step=0.05, format="%.2f", key="hc_r0"
    )
    v00 = (
        cc5.number_input(
            "v0_0 (sigma0 %)", value=20.0, min_value=0.1, step=0.5, format="%.1f", key="hc_v0"
        )
        / 100
    ) ** 2
    ...
    if st.button("Calibrar Heston", type="primary"):
        ...
        fig_cal.update_layout(
            **plotly_layout_cal("Heston calibrado vs sonrisa de mercado"), height=400
        )
        ...
        st.info(
            "Configura los puntos de mercado arriba y presiona **Calibrar Heston** para iniciar la optimizacion."
        )
