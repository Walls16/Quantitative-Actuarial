...
def _inputs_tasa_efectiva(sufijo: str):
    im = (
        st.number_input(
            "Tasa efectiva periódica ($i_m$) %", value=1.0, step=0.1, key=f"im_eff_{sufijo}"
        )
        / 100
    )
    n = st.number_input("Años ($n$)", min_value=0.0, value=5.0, step=1.0, key=f"n_eff_{sufijo}")
    m = st.number_input(
        "Periodos por año ($m$)", min_value=1.0, value=12.0, step=1.0, key=f"m_eff_{sufijo}"
    )
    return im, n * m, m
...
def _inputs_tasa_nominal(sufijo: str):
    i_nom = (
        st.number_input(
            "Tasa nominal anual ($i^{(m)}$) %", value=12.0, step=0.1, key=f"inom_{sufijo}"
        )
        / 100
    )
    n = st.number_input("Años ($n$)", min_value=0.0, value=5.0, step=1.0, key=f"n_nom_{sufijo}")
    m = st.number_input(
        "Periodos por año ($m$)", min_value=1.0, value=12.0, step=1.0, key=f"m_nom_{sufijo}"
    )
...
def _inputs_tasa_efectiva_pq(sufijo: str):
    im = (
        st.number_input("Tasa efec. interés ($i_m$) %", value=1.0, step=0.1, key=f"im_geo_{sufijo}")
        / 100
    )
    qm = (
        st.number_input(
            "Tasa efec. crecimiento ($q_m$) %", value=0.5, step=0.1, key=f"qm_geo_{sufijo}"
        )
        / 100
    )
    n = st.number_input("Años ($n$)", min_value=0.0, value=5.0, step=1.0, key=f"n_geo_{sufijo}")
    m = st.number_input(
        "Periodos por año ($m$)", min_value=1.0, value=12.0, step=1.0, key=f"m_geo_{sufijo}"
    )
    return im, qm, n * m, m
...
def _inputs_tasa_nominal_pq(sufijo: str):
    i_nom = (
        st.number_input(
            "Tasa nom. interés ($i^{(m)}$) %", value=12.0, step=0.1, key=f"inom_geo_{sufijo}"
        )
        / 100
    )
    q_nom = (
        st.number_input(
            "Tasa nom. crecimiento ($q^{(m)}$) %", value=5.0, step=0.1, key=f"qnom_geo_{sufijo}"
        )
        / 100
    )
    n = st.number_input("Años ($n$)", min_value=0.0, value=5.0, step=1.0, key=f"n_ngeo_{sufijo}")
    m = st.number_input(
        "Periodos por año ($m$)", min_value=1.0, value=12.0, step=1.0, key=f"m_ngeo_{sufijo}"
    )
...
with tab_vf:
    ...
    tipo_vf = st.radio(
        "Tipo de Renta:",
        ["Constantes Periódicas", "Crecientes Geométricas", "Crecientes Aritméticas"],
        horizontal=True,
        key="radio_tipo_vf",
    )
    separador()
    ...
    if tipo_vf == "Constantes Periódicas":
        escenario = st.selectbox(
            "Selecciona el escenario de capitalización:",
            [
        ...
        c1, c2 = st.columns(2)
        with c1:
            R_vf = st.number_input(
                "Pago periódico ($R$)", min_value=0.0, value=1_000.0, step=100.0, key="R_vf_const"
            )
            ...
            if escenario == "Vencidas a tasa efectiva im":
                ...
                i_nom_vf = (
                    st.number_input(
                        "Tasa nominal ($i^{(m)}$) %", value=12.0, step=0.1, key="inom_vf_p"
                    )
                    / 100
                )
                ...
                p_pag = st.number_input(
                    "Pagos por año ($p$)", min_value=1.0, value=4.0, step=1.0, key="p_vf"
                )
                ...
                R_anual = st.number_input(
                    "Flujo anual total ($\\bar{R}$)",
                    min_value=0.0,
                    value=12_000.0,
                    step=1_000.0,
                    key="R_cont_vf",
                ...
                tipo_t = st.radio(
                    "Ingresar tasa como:",
                    ["Tasa instantánea (δ)", "Tasa efectiva anual (i)"],
                    horizontal=True,
                    key="tipo_cont_vf",
                )
                n_cont = st.number_input(
                    "Años ($n$)", min_value=0.0, value=5.0, step=1.0, key="n_cont_vf"
                )
        ...
        with paso_a_paso():
            ...
            if "nominal i(m)" in escenario and "pagaderas" not in escenario:
                anticipada_str = (
                    rf"\left(1+\frac{{{i_nom_vf:.4f}}}{{{m_cap:g}}}\right)"
                    if "Anticipadas" in escenario
                    else ""
                )
                st.latex(
                    rf"VF = {R_vf:,.2f} \left[ \frac{{\left(1+\frac{{{i_nom_vf:.4f}}}{{{m_cap:g}}}\right)^{{{nm_vf:g}}} - 1}}{{\frac{{{i_nom_vf:.4f}}}{{{m_cap:g}}}}} \right]"
                    + anticipada_str
                )
                anticipada_str2 = rf"(1+{im_vf:.6f})" if "Anticipadas" in escenario else ""
                st.latex(
                    rf"VF = {R_vf:,.2f} \left[ \frac{{(1+{im_vf:.6f})^{{{nm_vf:g}}} - 1}}{{{im_vf:.6f}}} \right]"
                    + anticipada_str2
                )
                ...
                st.latex(
                    rf"VF = {R_vf:,.2f} \left[ \frac{{{cap_n:.6f} - 1}}{{{im_vf:.6f}}} \right]"
                    + anticipada_str2
                )
                ...
                if "pagaderas" in escenario:
                    st.latex(
                        rf"i_p = \left(1 + \frac{{{i_nom_vf:.4f}}}{{{m_cap:g}}}\right)^{{\frac{{{m_cap:g}}}{{{p_pag:g}}}}} - 1 = {i_p:.6f}"
                    )
                ...
                if "Anticipadas" in escenario:
                    st.latex(
                        rf"VF = {R_vf:,.2f} \left[ \frac{{(1 + {im_vf:.6f})^{{{nm_vf:g}}} - 1}}{{{im_vf:.6f}}} \right] (1 + {im_vf:.6f})"
                    )
                    st.latex(
                        rf"VF = {R_vf:,.2f} \left[ \frac{{{cap_n:.6f} - 1}}{{{im_vf:.6f}}} \right] ({1 + im_vf:.6f})"
                    )
                ...
                if tipo_t == "Tasa instantánea (δ)":
                    ...
                    st.latex(
                        rf"VF = {R_anual:,.2f} \left[ \frac{{e^{{({delta_vf:.4f} \times {n_cont:g})}} - 1}}{{{delta_vf:.4f}}} \right]"
                    )
                    st.latex(
                        rf"VF = {R_anual:,.2f} \left[ \frac{{{cap_n:.6f} - 1}}{{{delta_vf:.4f}}} \right]"
                    )
                    ...
                    factor = (cap_n - 1) / delta_vf
                    st.latex(
                        rf"VF = {R_anual:,.2f} \left[ \frac{{(1 + {i_eff_vf:.4f})^{{{n_cont:g}}} - 1}}{{{delta_vf:.6f}}} \right]"
                    )
                    st.latex(
                        rf"VF = {R_anual:,.2f} \left[ \frac{{{cap_n:.6f} - 1}}{{{delta_vf:.6f}}} \right]"
                    )
                    st.latex(rf"VF = {R_anual:,.2f} [{factor:.6f}]")

            themed_success(
                f"<div style='{css_paso}'><span style='{math_style}'>VF</span> = ${vf_res:,.4f}</div>"
            )
    ...
    # ──────────────────────────────────────────────
        ...
        c1, c2 = st.columns(2)
        with c1:
            R1_vf = st.number_input(
                "Primer pago ($R_1$)", min_value=0.0, value=1_000.0, step=100.0, key="R1_geo_vf"
            )
        ...
        with c2:
            ...
            if im_geo != qm_geo:
                st.latex(
                    rf"VF = R_1 \left[ \frac{{(1+{str_i})^{{nm}} - (1+{str_q})^{{nm}}}}{{{str_i} - {str_q}}} \right]"
                )
            else:
        ...
        with paso_a_paso():
            if im_geo != qm_geo:
                ...
                den = im_geo - qm_geo
                st.latex(
                    rf"VF = {R1_vf:,.2f} \left[ \frac{{(1 + {val_i})^{{{nm_geo:g}}} - (1 + {val_q})^{{{nm_geo:g}}}}}{{{val_i} - {val_q}}} \right]"
                )
                st.latex(
                    rf"VF = {R1_vf:,.2f} \left[ \frac{{{num1:.6f} - {num2:.6f}}}{{{den:.6f}}} \right]"
                )
                st.latex(rf"VF = {R1_vf:,.2f} [{((num1 - num2) / den):.6f}]")
            else:
                st.latex(rf"VF = nm \cdot R_1 (1+{str_i})^{{nm-1}}")
                st.latex(
                    rf"VF = {nm_geo:g} \times {R1_vf:,.2f} \times (1 + {val_i})^{{{nm_geo:g}-1}}"
                )
                st.latex(rf"VF = {nm_geo * R1_vf:,.2f} \times {(1 + im_geo) ** (nm_geo - 1):.6f}")
            themed_success(
                f"<div style='{css_paso}'><span style='{math_style}'>VF</span> = ${vf_geo:,.4f}</div>"
            )
    ...
    # ──────────────────────────────────────────────
        ...
        c1, c2 = st.columns(2)
        with c1:
            R1_arit = st.number_input(
                "Primer pago ($R_1$)", min_value=0.0, value=1_000.0, step=100.0, key="R1_arit_vf"
            )
        ...
        with c2:
            ...
            st.latex(
                rf"VF = R_1 \left[\frac{{(1+{str_i_a})^{{nm}}-1}}{{{str_i_a}}}\right] + \frac{{G}}{{{str_i_a}}}\left(\left[\frac{{(1+{str_i_a})^{{nm}}-1}}{{{str_i_a}}}\right] - nm\right)"
            )
        ...
        with paso_a_paso():
            ...
            st.latex(
                rf"VF = {R1_arit:,.2f} \left[\frac{{(1+{val_i_a})^{{{nm_arit:g}}}-1}}{{{val_i_a}}}\right] + \frac{{{G_vf:,.2f}}}{{{val_i_a}}}\left(\left[\frac{{(1+{val_i_a})^{{{nm_arit:g}}}-1}}{{{val_i_a}}}\right] - {nm_arit:g}\right)"
            )
            st.latex(
                rf"VF = {R1_arit:,.2f} \left[\frac{{{cap_n:.6f}-1}}{{{val_i_a}}}\right] + {G_vf / im_arit:,.2f} \left(\left[\frac{{{cap_n:.6f}-1}}{{{val_i_a}}}\right] - {nm_arit:g}\right)"
            )
            st.latex(
                rf"VF = {R1_arit:,.2f} [{sn:.6f}] + {G_vf / im_arit:,.2f} ({sn:.6f} - {nm_arit:g})"
            )
            st.latex(rf"VF = {t1_val:,.2f} + {t2_val:,.2f}")
            themed_success(
                f"<div style='{css_paso}'><span style='{math_style}'>VF</span> = ${vf_arit:,.4f}</div>"
            )
...
with tab_vp:
    ...
    if tipo_vp == "Constantes Periódicas":
        escenario_vp = st.selectbox(
            "Selecciona el escenario de actualización:",
            [
                "Vencidas a tasa efectiva im",
                "Anticipadas a tasa efectiva im",
                "Vencidas a tasa nominal i(m)",
        ...
        c1, c2 = st.columns(2)
        with c1:
            R_vp = st.number_input(
                "Pago periódico ($R$)", min_value=0.0, value=1_000.0, step=100.0, key="R_vp_const"
            )
            ...
            if escenario_vp == "Vencidas a tasa efectiva im":
                ...
                im_vp = (
                    st.number_input(
                        "Tasa efectiva periódica ($i_m$) %", value=1.0, step=0.1, key="im_perp_e"
                    )
                    / 100
                )
                nm_vp = 0
                ...
                i_nom_pp = (
                    st.number_input(
                        "Tasa nominal ($i^{(m)}$) %", value=12.0, step=0.1, key="inom_perp_n"
                    )
                    / 100
                )
                m_pp = st.number_input(
                ...
                i_nom_vp2 = (
                    st.number_input(
                        "Tasa nominal ($i^{(m)}$) %", value=12.0, step=0.1, key="inom_vp_p"
                    )
                    / 100
                )
                m_cap_vp = st.number_input(
                    "Capitalizaciones por año ($m$)",
                    min_value=1.0,
                    value=12.0,
                    step=1.0,
                ...
                p_pag_vp = st.number_input(
                    "Pagos por año ($p$)", min_value=1.0, value=4.0, step=1.0, key="p_vp"
                )
                ...
                n_cont_vp = st.number_input(
                    "Años ($n$)", min_value=0.0, value=5.0, step=1.0, key="n_cont_vp"
                )
        ...
        with paso_a_paso():
            ...
            if (
                "nominal i(m)" in escenario_vp
                and "pagaderas" not in escenario_vp
                and "Perpetuas" not in escenario_vp
            ):
                anticipada_str = (
                    rf"\left(1+\frac{{{i_nom_vp:.4f}}}{{{m_cap:g}}}\right)"
                    if "Anticipadas" in escenario_vp
                    else ""
                )
                st.latex(
                    rf"VP = {R_vp:,.2f} \left[ \frac{{1 - \left(1+\frac{{{i_nom_vp:.4f}}}{{{m_cap:g}}}\right)^{{-{nm_vp:g}}}}}{{\frac{{{i_nom_vp:.4f}}}{{{m_cap:g}}}}} \right]"
                    + anticipada_str
                )
                anticipada_str2 = rf"(1+{im_vp:.6f})" if "Anticipadas" in escenario_vp else ""
                st.latex(
                    rf"VP = {R_vp:,.2f} \left[ \frac{{1 - (1+{im_vp:.6f})^{{-{nm_vp:g}}}}}{{{im_vp:.6f}}} \right]"
                    + anticipada_str2
                )
                ...
                st.latex(
                    rf"VP = {R_vp:,.2f} \left[ \frac{{1 - {desc_n:.6f}}}{{{im_vp:.6f}}} \right]"
                    + anticipada_str2
                )
                st.latex(rf"VP = {R_vp:,.2f} [{factor:.6f}]" + anticipada_str2)

            elif escenario_vp in (
                "Vencidas a tasa efectiva im",
                "Anticipadas a tasa efectiva im",
                "Vencidas pagaderas p veces al año a tasa nominal i(m)",
            ):
                if "pagaderas" in escenario_vp:
                    st.latex(
                        rf"i_p = \left(1 + \frac{{{i_nom_vp2:.4f}}}{{{m_cap_vp:g}}}\right)^{{\frac{{{m_cap_vp:g}}}{{{p_pag_vp:g}}}}} - 1 = {i_p_vp:.6f}"
                    )
                ...
                if "Anticipadas" in escenario_vp:
                    st.latex(
                        rf"VP = {R_vp:,.2f} \left[ \frac{{1 - (1 + {im_vp:.6f})^{{-{nm_vp:g}}}}}{{{im_vp:.6f}}} \right] (1 + {im_vp:.6f})"
                    )
                    st.latex(
                        rf"VP = {R_vp:,.2f} \left[ \frac{{1 - {desc_n:.6f}}}{{{im_vp:.6f}}} \right] ({1 + im_vp:.6f})"
                    )
                ...
                if tipo_t_vp == "Tasa instantánea (δ)":
                    ...
                    st.latex(
                        rf"VP = {R_anual_vp:,.2f} \left[ \frac{{1 - e^{{-{delta_vp:.4f} \times {n_cont_vp:g}}}}}{{{delta_vp:.4f}}} \right]"
                    )
                    st.latex(
                        rf"VP = {R_anual_vp:,.2f} \left[ \frac{{1 - {desc_n:.6f}}}{{{delta_vp:.4f}}} \right]"
                    )
                    ...
                    factor = (1 - desc_n) / delta_vp
                    st.latex(
                        rf"VP = {R_anual_vp:,.2f} \left[ \frac{{1 - (1 + {i_eff_vp:.4f})^{{-{n_cont_vp:g}}}}}{{{delta_vp:.6f}}} \right]"
                    )
                    st.latex(
                        rf"VP = {R_anual_vp:,.2f} \left[ \frac{{1 - {desc_n:.6f}}}{{{delta_vp:.6f}}} \right]"
                    )
                    st.latex(rf"VP = {R_anual_vp:,.2f} [{factor:.6f}]")

            themed_info(
                f"<div style='{css_paso}'><span style='{math_style}'>VP</span> = ${vp_res:,.4f}</div>"
            )
    ...
    # ──────────────────────────────────────────────
        ...
        c1, c2 = st.columns(2)
        with c1:
            R1_vp = st.number_input(
                "Primer pago ($R_1$)", min_value=0.0, value=1_000.0, step=100.0, key="R1_geo_vp"
            )
        ...
        with c2:
            ...
            if im_geo_vp != qm_geo_vp:
                st.latex(
                    rf"VP = R_1 \left[ \frac{{1 - \left(\frac{{1+{str_q_gvp}}}{{1+{str_i_gvp}}}\right)^{{nm}}}}{{{str_i_gvp} - {str_q_gvp}}} \right]"
                )
            else:
        ...
        with paso_a_paso():
            if im_geo_vp != qm_geo_vp:
                ...
                st.latex(
                    rf"VP = {R1_vp:,.2f} \left[ \frac{{1 - \left(\frac{{1+{val_q_gvp}}}{{1+{val_i_gvp}}}\right)^{{{nm_geo_vp:g}}}}}{{{val_i_gvp} - {val_q_gvp}}} \right]"
                )
                st.latex(
                    rf"VP = {R1_vp:,.2f} \left[ \frac{{1 - ({frac:.6f})^{{{nm_geo_vp:g}}}}}{{{den:.6f}}} \right]"
                )
                st.latex(
                    rf"VP = {R1_vp:,.2f} \left[ \frac{{1 - {frac**nm_geo_vp:.6f}}}{{{den:.6f}}} \right]"
                )
            ...
            themed_info(
                f"<div style='{css_paso}'><span style='{math_style}'>VP</span> = ${vp_geo:,.4f}</div>"
            )
    ...
    # ──────────────────────────────────────────────
        ...
        c1, c2 = st.columns(2)
        with c1:
            R1_arit_vp = st.number_input(
                "Primer pago ($R_1$)", min_value=0.0, value=1_000.0, step=100.0, key="R1_arit_vp"
            )
        ...
        with c2:
            ...
            st.latex(
                rf"VP = R_1 \left[\frac{{1-(1+{str_i_av})^{{-nm}}}}{{{str_i_av}}}\right] + \frac{{G}}{{{str_i_av}}}\left(\left[\frac{{1-(1+{str_i_av})^{{-nm}}}}{{{str_i_av}}}\right] - nm(1+{str_i_av})^{{-nm}}\right)"
            )
        ...
        with paso_a_paso():
            ...
            st.latex(
                rf"VP = {R1_arit_vp:,.2f} \left[\frac{{1-(1+{val_i_av})^{{-{nm_arit_vp:g}}}}}{{{val_i_av}}}\right] + \frac{{{G_vp:,.2f}}}{{{val_i_av}}}\left(\left[\frac{{1-(1+{val_i_av})^{{-{nm_arit_vp:g}}}}}{{{val_i_av}}}\right] - {nm_arit_vp:g}(1+{val_i_av})^{{-{nm_arit_vp:g}}}\right)"
            )
            st.latex(
                rf"VP = {R1_arit_vp:,.2f} \left[\frac{{1-{desc_n:.6f}}}{{{val_i_av}}}\right] + {G_vp / im_arit_vp:,.2f}\left(\left[\frac{{1-{desc_n:.6f}}}{{{val_i_av}}}\right] - {nm_arit_vp:g}({desc_n:.6f})\right)"
            )
            st.latex(
                rf"VP = {R1_arit_vp:,.2f} [{an:.6f}] + {G_vp / im_arit_vp:,.2f} ({an:.6f} - {v_nm:.6f})"
            )
            st.latex(rf"VP = {t1v:,.2f} + {t2v:,.2f}")
            themed_info(
                f"<div style='{css_paso}'><span style='{math_style}'>VP</span> = ${vp_arit:,.4f}</div>"
            )
...
with tab_n:
    ...
    base_n = c_top1.selectbox(
        "Calcular $n$ desde:",
        ["Valor Futuro (Monto Acumulado)", "Valor Presente (Capital Inicial)"],
        key="sel_base_n",
    )
    tipo_renta_n = c_top2.selectbox(
        "Tipo de Renta:",
        ["Constante Periódica", "Creciente Geométrica", "Creciente Aritmética"],
        key="sel_tipo_n",
    )
    separador()

    tipo_tasa_n = st.radio(
        "Ingresar tasa como:",
        ["Tasa efectiva periódica", "Tasa nominal anual"],
        horizontal=True,
        key="tipo_tasa_n",
    ...
    with c3:
        ...
        R_n = st.number_input(
            "Primer pago ($R_1$)", min_value=0.01, value=1_000.0, step=100.0, key="n_pago"
        )

        if tipo_tasa_n == "Tasa efectiva periódica":
            im_n = (
                st.number_input(
                    "Tasa efectiva periódica ($i_m$) %", value=1.0, step=0.1, key="n_im_eff"
                )
                / 100
            )
            m_n = st.number_input(
                "Periodos por año ($m$)", min_value=1.0, value=12.0, step=1.0, key="n_m_eff"
            )
            str_i_n = r"i_m"
            val_i_n = f"{im_n:.4f}"
            str_q_n = r"q_m"
            ...
            i_nom_n = (
                st.number_input(
                    "Tasa nominal anual ($i^{(m)}$) %", value=12.0, step=0.1, key="n_inom"
                )
                / 100
            )
            m_n = st.number_input(
                "Periodos por año ($m$)", min_value=1.0, value=12.0, step=1.0, key="n_m_nom"
            )
            im_n = i_nom_n / m_n
        ...
        if tipo_renta_n == "Creciente Geométrica":
            if tipo_tasa_n == "Tasa efectiva periódica":
                qm_n = (
                    st.number_input(
                        "Tasa efectiva de crecimiento ($q_m$) %", value=0.5, step=0.1, key="n_qm"
                    )
                    / 100
                )
            else:
                q_nom_n = (
                    st.number_input(
                        "Tasa nominal crecimiento ($q^{(m)}$) %", value=5.0, step=0.1, key="n_qnom"
                    )
                    / 100
                )
                qm_n = q_nom_n / m_n
    ...
    with c4:
        ...
        if np.isnan(n_res_total):
            themed_error(
                "El monto objetivo es inalcanzable con la configuración de rentas y tasas actuales."
            )
        else:
            themed_info(
                f"<div style='{css_contenedor}'>"
            ...
            with paso_a_paso():
                ...
                if not usa_metodo_numerico:
                    if es_vf:
                        ...
                        st.latex(
                            rf"nm = \frac{{\ln\left(\frac{{{Meta:,.2f} \cdot {val_i_n}}}{{{R_n:,.2f}}} + 1\right)}}{{\ln(1+{val_i_n})}}"
                        )
                        st.latex(
                            rf"nm = \frac{{\ln({(Meta * im_n / R_n):.6f} + 1)}}{{\ln({1 + im_n:.6f})}}"
                        )
                        ...
                        st.latex(
                            rf"nm = \frac{{-\ln\left(1 - \frac{{{Meta:,.2f} \cdot {val_i_n}}}{{{R_n:,.2f}}}\right)}}{{\ln(1+{val_i_n})}}"
                        )
                        st.latex(
                            rf"nm = \frac{{-\ln(1 - {(Meta * im_n / R_n):.6f})}}{{\ln({1 + im_n:.6f})}}"
                        )
                        st.latex(rf"nm = \frac{{-\ln({num_val:.6f})}}{{{np.log(1 + im_n):.6f}}}")
                        st.latex(rf"nm = \frac{{-{np.log(num_val):.6f}}}{{{np.log(1 + im_n):.6f}}}")

                    themed_info(
                        f"<div style='{css_paso}'><span style='{math_style}'>nm</span> = {n_res_total:.4f}</div>"
                    )
                else:
                    st.latex(r"f(nm) = \text{Valor}_\text{renta}(nm) - \text{Objetivo} = 0")
                    themed_info(
                        f"<div style='{css_paso}'><span style='{math_style}'>nm</span> &approx; {n_res_total:.4f} (Numérico)</div>"
                    )
